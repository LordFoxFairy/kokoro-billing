...
import {
  createPostgresGrantExpiryService,
...
if (!Number.isSafeInteger(pollMs) || pollMs < 500 || pollMs > 300_000)
  throw new Error(
    "BILLING_EXPIRY_POLL_MS must be an integer between 500 and 300000",
  );
...
if (
  ...
  throw new Error(
    "BILLING_EXPIRY_METRICS_PORT must be an integer between 1 and 65535",
  );
const tenantId = process.env.BILLING_TENANT_ID;
if (!tenantId)
  throw new Error(
    "BILLING_TENANT_ID is required so every expiry batch is tenant-scoped",
  );
const configuredBatchId = process.env.BILLING_EXPIRY_BATCH_ID;
if (daemon && configuredBatchId)
  throw new Error("BILLING_EXPIRY_BATCH_ID is only valid for a one-shot retry");
const limit =
  process.env.BILLING_EXPIRY_LIMIT === undefined
    ? undefined
    : Number(process.env.BILLING_EXPIRY_LIMIT);
if (limit !== undefined && (!Number.isSafeInteger(limit) || limit <= 0))
  throw new Error("BILLING_EXPIRY_LIMIT must be a positive integer");
...
try {
  ...
  process.stderr.write(
    `kokoro-billing redis expiry hint unavailable during startup error=${error instanceof Error ? error.message : String(error)}\n`,
  );
}
const metricsServer = await startWorkerMetricsServer(
  metricsPort,
  process.env.BILLING_EXPIRY_METRICS_HOST ?? "127.0.0.1",
);
let stopping = false;
...
try {
  do {
    const result = await lease.runExclusive(
      "credit-hold-expiry",
      60,
      async () => {
        return runWithBillingContext(() => {
          ...
          return holds.then(async (holdResult) => ({
            ...holdResult,
            ...(await grants.expireExpiredGrants({
              tenantId,
              ...(limit === undefined ? {} : { limit }),
            })),
          }));
