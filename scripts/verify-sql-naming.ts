...
const tablePattern =
  /\bCREATE TABLE(?: IF NOT EXISTS)?\s+`?([a-zA-Z][a-zA-Z0-9_]*)`?/giu;
const namedObjectPattern = /\bCONSTRAINT\s+`?([a-zA-Z][a-zA-Z0-9_]*)`?/giu;

if (!/^CREATE TABLE/imu.test(sql))
  errors.push(
    "database/schema.sql: canonical schema must contain CREATE TABLE statements",
  );
const crossTableConstraintPattern = new RegExp(
  [["FOR", "EIGN"].join(""), "\\s+", "KEY"].join(""),
  "iu",
);
const referencePattern = new RegExp(["REF", "ERENCES"].join(""), "iu");
if (crossTableConstraintPattern.test(sql) || referencePattern.test(sql))
  errors.push(
    "database/schema.sql: cross-table database constraints are not allowed in Billing V1",
  );
if (
  /\bTIMESTAMP(?:\s*\(|\s+WITH|\s+WITHOUT)?\b/iu.test(
    sql.replace(/\bTIMESTAMPTZ\b/giu, ""),
  )
)
  errors.push("database/schema.sql: use TIMESTAMPTZ(3) for database instants");
if (/TIMESTAMPTZ(?!\(3\))/iu.test(sql))
  errors.push(
    "database/schema.sql: all timestamptz columns must use precision 3",
  );
