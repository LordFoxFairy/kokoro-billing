...
import {
  createPostgresCatalogAdminService,
...
type SeedDocument = {
  ...
  readonly plans?: readonly Omit<
    PublishCatalogPlanInput,
    "tenantId" | "operatorId" | "reason" | "idempotencyKey"
  >[];
...
if (process.env.ALLOW_BILLING_SEED !== "true")
  throw new Error("ALLOW_BILLING_SEED=true is required");
const raw = process.env.BILLING_SEED_JSON;
if (!raw) throw new Error("BILLING_SEED_JSON is required");

let seed: SeedDocument;
try {
  seed = JSON.parse(raw) as SeedDocument;
} catch {
  throw new Error("BILLING_SEED_JSON must be valid JSON");
}
if (!seed.tenantId || typeof seed.tenantId !== "string")
  throw new Error("BILLING_SEED_JSON.tenantId is required");
...
try {
  ...
  for (const account of seed.providerAccounts ?? []) {
    ...
    try {
      ...
      if (existing[0]?.tenant_id !== tenantId)
        throw new Error(
          `provider account ${account.provider}/${account.externalAccountRef} is already bound to another tenant`,
        );
      await connection.execute(
  ...
  for (const [index, plan] of (seed.plans ?? []).entries()) {
    const result = await catalog.publishPlan({
      ...plan,
      tenantId,
      operatorId,
      reason,
      idempotencyKey: `bootstrap:catalog:${digest}:${index}`,
    });
    console.log(
      JSON.stringify({ kind: "plan", key: result.key, revisionId: result.id }),
    );
  }
  if (seed.usagePrices && seed.usagePrices.length > 0) {
    const pricing = createPostgresUsagePricingAdminService(connection);
    const effectiveFrom = seed.effectiveFrom
      ? new Date(seed.effectiveFrom)
      : new Date(0);
    if (Number.isNaN(effectiveFrom.getTime()))
      throw new Error("BILLING_SEED_JSON.effectiveFrom must be an ISO date");
    const result = await pricing.publish({
      tenantId,
      operatorId,
      reason,
      effectiveFrom,
      rates: seed.usagePrices,
      idempotencyKey: `bootstrap:usage-pricing:${digest}`,
    });
    console.log(
      JSON.stringify({
        kind: "usage-pricing",
        revisionId: result.pricingRevisionId,
