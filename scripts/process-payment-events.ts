...
if (
  ...
  throw new Error(
    "BILLING_WORKER_METRICS_PORT must be an integer between 1 and 65535",
  );
const metricsServer = await startWorkerMetricsServer(
  metricsPort,
  process.env.BILLING_WORKER_METRICS_HOST ?? "127.0.0.1",
);
...
if (!Number.isSafeInteger(pollMs) || pollMs < 100 || pollMs > 60_000)
  throw new Error(
    "BILLING_WORKER_POLL_MS must be an integer between 100 and 60000",
  );
...
if (!Number.isSafeInteger(maxAttempts) || maxAttempts < 1 || maxAttempts > 100)
  throw new Error(
    "BILLING_OUTBOX_MAX_ATTEMPTS must be an integer between 1 and 100",
  );
const paymentWorker = new OutboxWorker(
  connection,
  "payment_outbox",
  30,
  ...
  const run = async (): Promise<{
    ...
    while (!stopping) {
      const result = await runWithBillingContext(() =>
        paymentWorker.processOnce(async (event) => {
          const providerEventId =
            typeof event.payload.providerEventId === "string"
              ? event.payload.providerEventId
              : null;
          if (!providerEventId)
            throw new Error("billing.provider_event_outbox_payload_invalid");
      ...
      try {
        const [rows] = await connection.query<
          (RowDataPacket & { age_seconds: number | string | null })[]
        >(
          `SELECT EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) AS age_seconds FROM payment_outbox WHERE event_type = 'PaymentProviderEventReceived' AND published_at IS NULL AND dead_lettered_at IS NULL ORDER BY created_at, outbox_id LIMIT 1`,
        );
        setOldestPendingAgeSeconds(
          rows[0]?.age_seconds === null || rows[0]?.age_seconds === undefined
            ? 0
            : Number(rows[0].age_seconds),
        );
      } catch {
        /* metrics must not stop the worker */
      }
      if (once) break;
