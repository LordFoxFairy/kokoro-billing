...
| Redis 故障 | key-presence marker/lease 非事实源；规范化 command 与 PostgreSQL receipt/fact 裁决 | startup/readiness 当前仍依赖 Redis，需演练 |
...
| Contract drift | owner OpenAPI、settlement/expiry shape、webhook matrix、metadata、route parity | 其余 body/response parity、historical breaking diff、machine provenance |
