...
| B3 / P0 / 三文档与ADR收敛 | Billing / Root / B1+B2 reviewers | AGENTS、三文档、ADR-0003、CURRENT、本任务板及README/INDEX导航 | 当前态/目标态、唯一schema、契约策略一致；两位reviewer局部放行B4，完整重写门待验 | 本文档提交作为B4设计基线 |
| B4 / P1 / 空库安装保护 | Billing / billing_owner（gpt-5.6-sol）/ B1+B2+Root | 仅scripts/apply-schema.ts、scripts/canonical-schema.ts、test/integration/schema-installation.test.ts；禁止改SQL/contract/依赖/src/文档 | 依赖B3；独立临时DB、TDD、非空/custom schema/并发/回滚/锁超时；主控提交并重跑全部gate | 待派工 |
...
- [ ] B4交付、两阶段review、Root主工作树重跑验证与commit。

## B4执行卡（仅离线安装局部修复）

入口与API不新增公开业务能力。设计引用TECHNICAL_DESIGN顶部B4，不重复专项手册；新增两个角色文件，不建新目录。

- [ ] 在`test/integration/schema-installation.test.ts`先写行为反例；用当前CLI证明已有view/custom schema等错误接受行为（记录RED）。
- [ ] 将安装职责提取为`scripts/canonical-schema.ts`的具名函数；入口只读环境/SQL并调用。输入是内部安装配置，不是HTTP DTO。
- [ ] 全部schema URL值只允许public；实际public存在、有权限；固定READ COMMITTED、UTC/search_path与预算。
- [ ] 同client事务/advisory lock后检查全部用户relations/独立types/functions，非空拒绝；DDL失败全回滚，不自动建/删schema。
- [ ] 两个真实独立安装者一成一拒；锁占用预算内失败；已有对象及payload不变；中途DDL失败后再次正常安装成功。
- [ ] 单文件测试独占随机database，管理连接只用于创建/删除本轮随机名；禁止重置DATABASE_URL所指共享库或FLUSH Redis。
- [ ] `pnpm lint`、`pnpm typecheck`、`pnpm build`、定向integration实际运行后交付文件清单；不操作Git index。
- [ ] 主控先检查规格符合性，再独立代码审查，再在自建临时库重跑所有gate。后续本仓实现优先续派同一负责人。
