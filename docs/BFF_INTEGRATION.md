...
- 重复 checkout 使用同一 tenant、subject、payload 和 `Idempotency-Key` 时由 Billing durable fact 返回原结果；同一 key 搭配不同
  规范化 payload 返回 `billing.idempotency_conflict`（409）。Redis 只写 scoped key-presence marker，不查看 payload 或裁决冲突。
