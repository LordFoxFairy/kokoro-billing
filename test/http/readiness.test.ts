...
const dependencies = (health: NonNullable<BillingHttpDependencies['health']>): BillingHttpDependencies => ({
  checkout: { create: async () => Promise.resolve(({ checkoutId: 'unused', status: 'created', amountMinor: 1, currency: 'USD', expiresAt: new Date('2030-01-01T00:00:00Z') })) },
  ...
  webhook: { accept: async () => Promise.resolve(({ providerEventId: 'unused', processingStatus: 'received' })) },
...
describe('Billing readiness dependency authority', () => {
  it('remains ready with Redis marked degraded when PostgreSQL is healthy', async () => {
    const server = createBillingServer(dependencies({
      ...
      redis: async () => { return Promise.reject(new Error('redis unavailable')); },
    }));
    try {
  ...
  it('is not ready when the PostgreSQL authority is unavailable', async () => {
    const server = createBillingServer(dependencies({
      postgres: async () => { return Promise.reject(new Error('postgres unavailable')); },
      redis: async () => Promise.resolve(undefined),
    }));
    try {
      const response = await server.inject({ method: 'GET', url: '/readyz' });
      expect(response.statusCode).toBe(503);
      expect(errorEnvelope.parse(response.json<unknown>()).error.code).toBe('billing.dependencies_not_ready');
    } finally {
      await server.close();
    }
  });
