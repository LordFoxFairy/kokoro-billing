...
const dependencies: BillingHttpDependencies = {
  ...
  resolveWebhookTenant: async (provider, accountRef) =>
    Promise.resolve(
      provider === "stripe" && accountRef === "acct-1" ? "tenant-1" : null,
    ),
  account: {
    getForSubject: async () =>
      Promise.resolve({
  ...
  accountRead: {
    ledgerForSubject: async () => Promise.resolve({ entries: [] }),
  },
  auth: {
    user: async (request) =>
      Promise.resolve(
...
describe("Billing HTTP surface", () => {
  it("exposes liveness, readiness and metrics without authentication", async () => {
    ...
    expect(dataEnvelope.parse(live.json<unknown>()).data).toMatchObject({
  ...
  it("reads the ledger subject from verified context instead of requiring a query subject", async () => {
    const response = await server.inject({
      method: "GET",
      url: "/v1/billing/me/credit-ledger?limit=20",
      headers: {
        "x-kokoro-tenant-id": "tenant-1",
        "x-kokoro-subject": "user-1",
      },
  ...
  it("uses the provider account registry as webhook tenant authority", async () => {
    ...
    expect(webhookCalls.at(-1)).toMatchObject({
      tenantId: "tenant-1",
      provider: "stripe",
      providerAccountRef: "acct-1",
    });
  ...
  it("rejects providers outside the production registry before webhook processing", async () => {
    ...
    expect(response.statusCode).toBe(400);
    expect(errorEnvelope.parse(response.json<unknown>()).error.code).toBe(
      "billing.provider_not_supported",
    );
  });
  ...
  it("accepts Alipay RSA2 only from the signed form body and ignores a query signature", async () => {
    ...
    const params = {
      notify_id: "notify-http-1",
      sign_type: "RSA2",
      app_id: "app-1",
      trade_status: "TRADE_SUCCESS",
      trade_no: "trade-http-1",
    };
    const signed = {
      ...params,
      sign: signAlipayNotification(params, privateKeyPem),
    };
    ...
    const alipayServer = createBillingServer({
      ...
      webhook: {
        accept: async (input) => {
          ...
          return Promise.resolve({
            providerEventId: "event-alipay-1",
            processingStatus: "received" as const,
          });
        },
      },
      parseWebhook: (provider, payload) =>
        parseProviderWebhook(registry, provider, payload),
      resolveWebhookTenant: async (provider, accountRef) =>
        Promise.resolve(
          provider === "alipay" && accountRef === "app-1" ? "tenant-1" : null,
        ),
...
describe("webhook fallback input types", () => {
  it.each(["id", "type"] as const)(
    "rejects non-string %s before calling inbox",
    async (field) => {
  ...
  it("preserves string, null and absent fallbacks and parsed-value precedence", async () => {
    ...
    delete fallbackDependencies.resolveWebhookTenant;
    const accepted: unknown[] = [];
    const webhook: BillingHttpDependencies["webhook"] = {
      accept: (input) => {
        accepted.push(input);
        return Promise.resolve({
          providerEventId: "fallback-event",
    ...
    try {
      ...
      const parsed = await parsedApp.inject({
        method: "POST",
        url: "/v1/webhooks/payment/stripe",
        payload: { id: {}, type: [], providerAccountRef: "acct-1" },
      });
