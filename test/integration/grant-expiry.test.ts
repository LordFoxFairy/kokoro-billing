...
integration("credit grant expiry", () => {
  it("expires unused grant balance transactionally and is replay-safe", async () => {
    ...
    try {
      await connection.execute(
        `INSERT INTO entitlement_credit_account (credit_account_id, tenant_id, subject_id, available_micros) VALUES ($1, $2, $3, 100)`,
      ...
      await expect(service.expireExpiredGrants({ tenantId })).resolves.toEqual({
        expiredGrantIds: [grantId],
      });
      await expect(service.expireExpiredGrants({ tenantId })).resolves.toEqual({
        expiredGrantIds: [],
      });
      const [grantRows] = await connection.query<
        (RowDataPacket & { remaining_micros: string; status: string })[]
      >(
        "SELECT remaining_micros, status FROM entitlement_credit_grant WHERE credit_grant_id = $1",
        [grantId],
      );
      ...
      const [journalRows] = await connection.query(
        "SELECT journal_id FROM entitlement_credit_journal WHERE tenant_id = $1 AND source_kind = 'grant_expiry' AND source_ref = $2",
        [tenantId, grantId],
      );
  ...
  it("defers expiry while an active hold still owns grant allocation", async () => {
    ...
    try {
      await connection.execute(
        `INSERT INTO entitlement_credit_account (credit_account_id, tenant_id, subject_id, available_micros, held_micros) VALUES ($1, $2, $3, 100, 30)`,
        [accountId, tenantId, `subject-${accountId}`],
      );
      await connection.execute(
        `INSERT INTO entitlement_credit_grant
      ...
      const [activeRows] = await connection.query<RowDataPacket[]>(
        "SELECT status, remaining_micros FROM entitlement_credit_grant WHERE credit_grant_id = $1",
        [grantId],
      );
      expect(activeRows[0]).toMatchObject({
        status: "active",
        remaining_micros: "100",
      });

      await connection.execute(
        `UPDATE entitlement_credit_hold SET status = 'released', released_micros = requested_micros WHERE credit_hold_id = $1`,
        [holdId],
      );
      await expect(service.expireExpiredGrants({ tenantId })).resolves.toEqual({
        expiredGrantIds: [grantId],
      ...
      await connection.execute(
        "DELETE FROM entitlement_credit_hold_allocation WHERE credit_grant_id = $1",
        [grantId],
      );
      await connection.execute(
        "DELETE FROM entitlement_credit_hold WHERE tenant_id = $1",
        [tenantId],
      );
      await connection.execute(
        "DELETE FROM entitlement_outbox WHERE tenant_id = $1",
  ...
  it("uses tenant-scoped application predicates instead of database relationship constraints", async () => {
    ...
    try {
      await connection.execute(
        `INSERT INTO entitlement_credit_account (credit_account_id, tenant_id, subject_id, available_micros) VALUES ($1, $2, $3, 10)`,
        [accountId, siteA, `subject-${accountId}`],
      );
      await connection.execute(
        `INSERT INTO entitlement_credit_grant
      ...
      await connection.execute(
        "DELETE FROM entitlement_credit_hold_allocation WHERE credit_hold_id = $1",
        [holdId],
      );
      await connection.execute(
      ...
      await connection.execute(
        "DELETE FROM entitlement_credit_grant WHERE credit_grant_id = $1",
        [grantId],
      );
      await connection.execute(
