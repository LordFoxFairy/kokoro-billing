...
integration("user credit account query", () => {
  it("returns only the account owned by the verified site and subject context", async () => {
    ...
    try {
      await createPostgresAdminGrantService(connection).grant({
        ...
        reason: "test",
        idempotencyKey: `query-${randomUUID()}`,
      });
      const query = createPostgresCreditAccountQueryService(connection);
      expect(await query.getForSubject(tenantId, subjectId)).toMatchObject({
        accountId,
        availableMicros: "10",
        heldMicros: "0",
      });
    ...
    try {
      ...
      for (const programKey of ["one", "two", "three"]) {
        await admin.grant({
          ...
          operatorId: "operator-1",
          reason: "test",
          idempotencyKey: `ledger-${programKey}-${randomUUID()}`,
        });
      }
      const query = createPostgresCreditAccountQueryService(connection);
      ...
      const second = await query.ledgerForSubject(
        tenantId,
        subjectId,
        2,
        first.nextCursor,
  ...
  it("does not emit a continuation cursor when the result count exactly equals the requested limit", async () => {
    ...
    try {
      ...
      const page = await createPostgresCreditAccountQueryService(
        connection,
      ).ledgerForSubject(tenantId, subjectId, 2);
  ...
  it("binds ledger cursors to tenant and subject scope", async () => {
    ...
    try {
      ...
      const first = await createPostgresCreditAccountQueryService(
        connection,
      ).ledgerForSubject(tenantId, firstSubjectId, 1);
      expect(first.nextCursor).toEqual(expect.any(String));
      await expect(
        createPostgresCreditAccountQueryService(connection).ledgerForSubject(
          tenantId,
          secondSubjectId,
          1,
          first.nextCursor,
  ...
  it("excludes a journal row whose tenant differs from the referenced account tenant", async () => {
    ...
    try {
      await createPostgresAdminGrantService(connection).grant({
        ...
        operatorId: "operator-1",
        reason: "test",
        idempotencyKey: `tenant-join-${randomUUID()}`,
      });
      ...
      const page = await createPostgresCreditAccountQueryService(
        connection,
      ).ledgerForSubject(tenantId, subjectId, 10);
      expect(page.entries).toHaveLength(1);
      expect(page.entries[0]).toMatchObject({
        deltaMicros: "10",
        balanceAfterMicros: "10",
      });
    } finally {
      await connection.execute(
        "DELETE FROM entitlement_credit_journal WHERE journal_id = $1",
        [contaminatedJournalId],
      );
      await connection.end();
    }
  });
