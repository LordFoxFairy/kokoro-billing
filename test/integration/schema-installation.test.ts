...
integration('canonical schema installation', () => {
  ...
  const install = (url = databaseUrl(database), sql = canonicalSql, lockTimeoutMs = 1_000, clientQueryTimeoutMs = 12_000) =>
    installCanonicalSchema({ databaseUrl: url, schemaSql: sql, lockTimeoutMs, statementTimeoutMs: 10_000, idleInTransactionTimeoutMs: 10_000, clientQueryTimeoutMs, closeTimeoutMs: 2_000 });
  ...
  async function expectNoPartialTable(): Promise<void> {
    const pool = new Pool({ connectionString: databaseUrl(database), max: 1 });
    try {
      const result = await pool.query<{ relation: string | null }>("SELECT pg_catalog.to_regclass('public.partial')::text AS relation");
  ...
  it('installs exactly 35 canonical tables and rejects a repeat installation', async () => {
    ...
    const pool = new Pool({ connectionString: databaseUrl(database), max: 1 });
    try {
      const result = await pool.query<{ count: string }>("SELECT count(*)::text AS count FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relkind IN ('r','p')");
  ...
  it.each([
    ['non-public table', 'CREATE SCHEMA occupied; CREATE TABLE occupied.item(id int)'],
    ['public view', 'CREATE VIEW public.item AS SELECT 1 AS id'],
    ['public sequence', 'CREATE SEQUENCE public.item_seq'],
    ['public materialized view', 'CREATE MATERIALIZED VIEW public.item AS SELECT 1 AS id'],
    ['public foreign table', "CREATE EXTENSION file_fdw WITH SCHEMA pg_catalog; CREATE SERVER fixture FOREIGN DATA WRAPPER file_fdw; CREATE FOREIGN TABLE public.item(id int) SERVER fixture OPTIONS (filename '/tmp/billing-fixture')"],
    ...
    ['function', 'CREATE FUNCTION public.item() RETURNS int LANGUAGE sql AS $$ SELECT 1 $$'],
    ['procedure', 'CREATE PROCEDURE public.item() LANGUAGE sql AS $$ SELECT 1 $$'],
    ...
    try {
      ...
      const billing = await pool.query<{ count: string }>("SELECT count(*)::text AS count FROM pg_catalog.pg_class c JOIN pg_catalog.pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND (c.relname LIKE 'payment_%' OR c.relname LIKE 'entitlement_%')");
  ...
  it('rejects every non-public schema URL value before connecting', async () => {
    ...
    await expect(install(`${unreachable}?schema=public&schema=private`)).rejects.toThrow(/schema.*public/iu);
    await expect(install(`${unreachable}?schema=private&schema=public`)).rejects.toThrow(/schema.*public/iu);
    await expect(install(`${unreachable}?schema=private`)).rejects.toThrow(/schema.*public/iu);
  });
  ...
  it('verifies CREATE privilege using the effective connection role', async () => {
    ...
    await expect(install(databaseUrl(database, `?options=${options}`))).rejects.toThrow(/CREATE privilege/iu);
  });
  ...
  it('overrides URI options search_path and installs only in public', async () => {
    ...
    const pool = new Pool({ connectionString: databaseUrl(database), max: 1 });
    try {
      const result = await pool.query<{ schema_name: string }>("SELECT DISTINCT n.nspname AS schema_name FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE c.relname LIKE 'payment_%' OR c.relname LIKE 'entitlement_%'");
  ...
  it('rolls back a mid-DDL failure and permits a later canonical installation', async () => {
    await expect(install(undefined, 'CREATE TABLE public.first(id int); SELECT missing_function();')).rejects.toThrow();
    await install();
  });
  ...
  it('serializes two independent installers so one succeeds and one rejects non-empty', async () => {
    ...
    if (rejected?.status === 'rejected') expect(String(rejected.reason)).toMatch(/non-empty/iu);
  });

  it('fails within the lock budget and releases its client', async () => {
    const blocker = new Pool({ connectionString: databaseUrl(database), max: 1 });
    try {
      await blocker.query('BEGIN');
      await blocker.query("SELECT pg_advisory_xact_lock(hashtextextended('kokoro-billing:canonical-schema', 0))");
      const started = Date.now();
      await expect(install(undefined, canonicalSql, 100)).rejects.toThrow(/lock timeout|canceling statement/iu);
  ...
  it('turns a terminated checked-out backend into a controlled rejection and recovers', async () => {
    ...
    const pending = install(undefined, `CREATE TABLE public.partial(id int); SELECT pg_sleep(10) /* ${marker} */`);
    ...
    await expect(pending).rejects.toThrow(/terminating connection|connection terminated/iu);
    await expectNoPartialTable();
    await install();
  });

  it('destroys and releases a client when the JavaScript query deadline expires', async () => {
    await expect(install(undefined, 'CREATE TABLE public.partial(id int); SELECT pg_sleep(1)', 1_000, 100)).rejects.toThrow(/query exceeded 100ms/iu);
    await expectNoPartialTable();
    await install();
  });
});
