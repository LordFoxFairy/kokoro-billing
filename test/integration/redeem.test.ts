...
import {
  createPostgresRedeemAdminService,
  createPostgresRedeemService,
  ...
  it("issues plaintext once, stores only HMAC, and atomically grants once", async () => {
    ...
    try {
      const campaign = await admin.createCampaign({
        tenantId,
        campaignKey: `test-${randomUUID()}`,
        programKey: "ai-pro",
        creditMicros: 1000,
        maxRedemptions: 10,
        operatorId: "test-operator",
        reason: "integration test",
        idempotencyKey: `campaign-${randomUUID()}`,
      });
      const batchKey = `batch-${randomUUID()}`;
      const batch = await admin.issueCodes({
        tenantId,
      ...
      const replayBatch = await admin.issueCodes({
        tenantId,
        campaignId: campaign.campaignId,
        count: 1,
        operatorId: "test-operator",
      ...
      const [stored] = await connection.query<
        (RowDataPacket & { code_hash: string })[]
      >("SELECT code_hash FROM entitlement_redeem_code WHERE batch_id = $1", [
        batch.batchId,
      ]);
      const plaintext = assertDefined(batch.codes[0]);
      expect(assertDefined(stored[0]).code_hash).toBe(
        hashRedeemCode(plaintext, secret),
      );
      const input = {
        tenantId,
      ...
      await expect(
        redeem.redeem({ ...input, idempotencyKey: `other-${randomUUID()}` }),
      ).rejects.toThrow("billing.redeem_invalid");
    } finally {
