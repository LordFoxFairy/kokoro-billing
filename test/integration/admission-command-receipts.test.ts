...
import { createPostgresBillingAdmissionService, createPostgresBillingSettlementService, createPostgresUsageSettlementService } from '../../src/infrastructure/postgres/create-postgres-services.js';
...
const createHarness = async (): Promise<{
  ...
  const usage = {
    authorizeUsage: async () => Promise.resolve(({ holdId: randomUUID(), allocations: [] })),
    settleUsage: async () => Promise.resolve(({ settlementId: randomUUID(), capturedMicros: 10, releasedMicros: 0 })),
    releaseUsage: async (input: { readonly holdId: string }) => Promise.resolve(({ holdId: input.holdId, releasedMicros: 10 })),
  ...
  const server = createBillingServer({
    ...
    auth: {
      ...
      internal: async (request) => {
        ...
        return Promise.resolve(typeof requestedTenant === 'string' && typeof serviceId === 'string'
...
integration('admission and execution command receipts', () => {
  it('checks the capture receipt before a terminal admission replay', async () => {
    ...
    try {
      ...
      expect(dataEnvelope.parse(identityReplay.json<unknown>()).data).toEqual(dataEnvelope.parse(first.json<unknown>()).data);
  ...
  it('uses event ID as command identity while binding the required HTTP key', async () => {
    ...
    try {
      ...
      expect(errorEnvelope.parse(keyDrift.json<unknown>()).error.code).toBe('billing.idempotency_conflict');
...
integration('execution receipt provider reference input types', () => {
  it.each([
    ['string', 'provider-operation', true, true], ['null', null, true, true], ['missing', undefined, true, true],
    ['object', {}, false, true], ['array', [], false, true], ['boolean', true, false, true], ['number', 123, false, true],
    ['default UUID characterization: known 22001 rollback, not successful capture', 'provider-operation', true, false],
  ] as const)('validates %s before debit and capture receipt completion', async (_name, providerRef, valid, shortHold) => {
    const connection = await createBillingConnection(requiredDatabaseUrl());
    const tenantId = randomUUID();
    ...
    try {
      ...
      await settlement.recordSettlement({ tenantId, settlementId, idempotencyKey: randomUUID(), externalPaymentRef: randomUUID(), amountMinor: 100, currency: 'USD' });
      await settlement.fulfillSettlement({ tenantId, settlementId, accountId, subjectId: accountId, programKey: 'typed-receipt', grantMicros: 100 });
      const hold = await usage.authorizeUsage({ tenantId, accountId, idempotencyKey: randomUUID(), requestedMicros: 10, featureKey: 'model.request' });
      ...
      if (shortHold) {
        ...
        await connection.withTransaction(async () => {
          await connection.execute('UPDATE entitlement_credit_hold SET credit_hold_id = $1 WHERE tenant_id = $2 AND credit_hold_id = $3', [holdId, tenantId, hold.holdId]);
          await connection.execute('UPDATE entitlement_credit_hold_allocation SET credit_hold_id = $1 WHERE tenant_id = $2 AND credit_hold_id = $3', [holdId, tenantId, hold.holdId]);
        });
      }
      const seeded = await seedAdmission(connection, tenantId);
      await connection.execute('UPDATE entitlement_billing_admission SET hold_id = $1 WHERE tenant_id = $2 AND admission_id = $3', [holdId, tenantId, seeded.admissionId]);
      const eventId = randomUUID();
      await admission.recordExecutionEvent({ tenantId, eventId, eventType: 'execution.accepted', invocationId: seeded.invocationId, executionId: seeded.executionId, occurredAt: new Date('2026-09-08T12:00:00.000Z'), receiptSchemaVersion: '1', idempotencyKey: randomUUID(),
        receipt: providerRef === undefined ? {} : { provider_operation_ref: providerRef } });
      const captured = valid && shortHold;
      if (!shortHold) await expect(admission.processExecutionEvent(tenantId, eventId)).rejects.toMatchObject({ code: '22001' });
      else if (valid) await admission.processExecutionEvent(tenantId, eventId);
      else await expect(admission.processExecutionEvent(tenantId, eventId)).rejects.toThrow('billing.execution_receipt_invalid');
      const [accounts] = await connection.query<(RowDataPacket & { available_micros: string; held_micros: string })[]>('SELECT available_micros, held_micros FROM entitlement_credit_account WHERE tenant_id = $1 AND credit_account_id = $2', [tenantId, accountId]);
      expect(accounts).toEqual([{ available_micros: '90', held_micros: captured ? '0' : '10' }]);
      const [events] = await connection.query<(RowDataPacket & { status: string })[]>('SELECT status FROM entitlement_execution_event WHERE tenant_id = $1 AND event_id = $2', [tenantId, eventId]);
      expect(events).toEqual([{ status: captured ? 'processed' : 'received' }]);
      const [admissions] = await connection.query<(RowDataPacket & { status: string; accepted_provider_ref: string | null })[]>('SELECT status, accepted_provider_ref FROM entitlement_billing_admission WHERE tenant_id = $1 AND admission_id = $2', [tenantId, seeded.admissionId]);
      expect(admissions).toEqual([{ status: captured ? 'captured' : 'held', accepted_provider_ref: captured ? providerRef ?? 'event' : null }]);
      const [receipts] = await connection.query<(RowDataPacket & { status: string })[]>("SELECT status FROM entitlement_billing_command_receipt WHERE tenant_id = $1 AND command_name = 'CaptureAdmission'", [tenantId]);
