...
integration('provider webhook inbox', () => {
  it('stores a signed provider event once and returns the same fact on replay', async () => {
    ...
    try {
      ...
      const [events] = await connection.query('SELECT provider_event_id FROM payment_provider_event WHERE tenant_id = $1 AND external_event_id = $2', [tenantId, externalEventId]);
  ...
  it('rejects the same external event id with a different payload', async () => {
    ...
    const externalEventId = `evt-${randomUUID()}`;
    try {
      await service.accept({ tenantId, provider: 'mock', externalEventId, eventType: 'payment.succeeded', rawPayload: { id: externalEventId, amount: 1000 }, signatureValid: true });
      await expect(service.accept({ tenantId, provider: 'mock', externalEventId, eventType: 'payment.succeeded', rawPayload: { id: externalEventId, amount: 2000 }, signatureValid: true })).rejects.toThrow('billing.idempotency_conflict');
    } finally {
      await connection.end();
    }
  });
  ...
  it('requeues a failed event with a durable audited command', async () => {
    ...
    try {
      ...
      const [audit] = await connection.query('SELECT audit_event_id FROM entitlement_audit_event WHERE tenant_id = $1 AND action = \'provider_event_retry\'', [tenantId]);
  ...
  it('lists only the current site and supports processing-status filtering', async () => {
    ...
    try {
      const failed = await inbox.accept({ tenantId, provider: 'mock', externalEventId: `evt-${randomUUID()}`, eventType: 'payment.failed', rawPayload: { ok: false }, signatureValid: true });
      const failedAgain = await inbox.accept({ tenantId, provider: 'mock', externalEventId: `evt-${randomUUID()}`, eventType: 'payment.failed', rawPayload: { ok: false }, signatureValid: true });
      await inbox.accept({ tenantId, provider: 'mock', externalEventId: `evt-${randomUUID()}`, eventType: 'payment.succeeded', rawPayload: { ok: true }, signatureValid: true });
      await inbox.accept({ tenantId: otherSiteId, provider: 'mock', externalEventId: `evt-${randomUUID()}`, eventType: 'payment.failed', rawPayload: { ok: false }, signatureValid: true });
