...
integration("provider account tenant routing", () => {
  it("resolves a provider account to exactly one active site", async () => {
    ...
    try {
      ...
      await expect(
        accounts.resolveTenantId("stripe", externalAccountRef),
      ).resolves.toBe(tenantId);
      await expect(
        accounts.resolveTenantId("stripe", "unknown-account"),
      ).resolves.toBeNull();
      await expect(
        connection.execute(
          `INSERT INTO payment_provider_account (provider_account_id, tenant_id, provider, external_account_ref, status)
      ...
      await connection.execute(
        "DELETE FROM payment_provider_account WHERE tenant_id = $1",
        [tenantId],
      );
