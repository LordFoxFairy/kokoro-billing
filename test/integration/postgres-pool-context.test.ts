  ...
  it("assigns concurrent transactions to independent physical sessions", async () => {
    ...
    try {
      const ids = await Promise.all(
        [1, 2].map(() =>
          runWithBillingContext(async () => {
            await connection.beginTransaction();
            try {
              const [rows] = await connection.query<
                (RowDataPacket & { connection_id: number })[]
  ...
  it("propagates the context through a Fastify request lifecycle", async () => {
    ...
    try {
      const responses = await Promise.all(
        [1, 2].map(() => app.inject({ method: "GET", url: "/" })),
      );
      ...
      expect(
        z
          .object({ connectionId: z.number() })
          .parse(assertDefined(responses[0]).json<unknown>()).connectionId,
      ).not.toBe(
        z
          .object({ connectionId: z.number() })
          .parse(assertDefined(responses[1]).json<unknown>()).connectionId,
      );
    } finally {
      await app.close();
      await connection.end();
