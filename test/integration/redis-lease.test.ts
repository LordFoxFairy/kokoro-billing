...
integration('billing Redis lease', () => {
  it('allows one leader and releases the lease after completion', async () => {
    ...
    const first = new RedisLease(url!, namespace);
    const second = new RedisLease(url!, namespace);
    await first.connect();
    await second.connect();
    try {
      const held = await first.runExclusive('sweep', 30, async () => {
        expect(await second.runExclusive('sweep', 30, async () => 'unexpected')).toBeUndefined();
        return 'done';
      });
      expect(held).toBe('done');
      expect(await second.runExclusive('sweep', 30, async () => 'reacquired')).toBe('reacquired');
    } finally {
      await first.close();
      await second.close();
    }
  ...
  it('renews a long-running lease until the owner finishes', async () => {
    ...
    try {
      const held = await first.runExclusive('worker', 1, async () => {
        ...
        expect(await second.runExclusive('worker', 1, async () => 'unexpected')).toBeUndefined();
        return 'done';
      });
      expect(held).toBe('done');
      expect(await second.runExclusive('worker', 1, async () => 'reacquired')).toBe('reacquired');
    } finally {
      await first.close();
      await second.close();
    }
