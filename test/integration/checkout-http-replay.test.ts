...
integration("checkout durable HTTP replay", () => {
  it("replays reordered canonical JSON after the offer is disabled and rejects payload drift", async () => {
    ...
    const server = createBillingServer({
      ...
      reversal: {
        recordReversal: async () => Promise.resolve("unused-refund"),
      },
      webhook: {
        accept: async () =>
          Promise.resolve({
      ...
      auth: {
        user: async (request) =>
          Promise.resolve(
            request.headers["x-kokoro-tenant-id"] === tenantId
              ? { tenantId, subjectId }
              : null,
          ),
    ...
    const firstPayload = {
      ...
      quote_snapshot: {
        ...
        tiers: [
          { name: "base", amount: 1000 },
          { amount: 2000, name: "plus" },
        ],
      },
    };
    ...
    try {
      ...
      const reorderedReplay = await server.inject({
        ...
        payload: {
          quote_snapshot: {
            tiers: [
              { amount: 1000, name: "base" },
              { name: "plus", amount: 2000 },
            ],
            metadata: {
              display: { rank: 1, badge: "popular" },
              region: "global",
            },
            credit_micros: "1000",
            key: "pro",
      ...
      expect(
        dataEnvelope.parse(reorderedReplay.json<unknown>()).data.checkout_id,
      ).toBe(dataEnvelope.parse(first.json<unknown>()).data.checkout_id);
