  ...
  it("requires a reason and creates one auditable grant/journal on replay", async () => {
    ...
    try {
      ...
      const [grants] = await connection.query(
        "SELECT credit_grant_id FROM entitlement_credit_grant WHERE credit_grant_id = $1",
        [first.grantId],
      );
      const [audit] = await connection.query(
        "SELECT audit_event_id FROM entitlement_audit_event WHERE resource_id = $1",
        [first.grantId],
      );
      expect(grants).toHaveLength(1);
      expect(audit).toHaveLength(1);
      await expect(
        service.grant({ ...input, amountMicros: 26 }),
    ...
    try {
      ...
      await expect(
        service.grant({
          ...
          operatorId: "operator-1",
          reason: "support adjustment",
          idempotencyKey: `admin-grant-mismatch-${randomUUID()}`,
        }),
      ).rejects.toThrow("billing.credit_account_mismatch");
    } finally {
      await connection.execute(
