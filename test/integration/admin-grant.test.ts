...
integration('admin credit grant', () => {
  it('requires a reason and creates one auditable grant/journal on replay', async () => {
    ...
    const input = { siteId: randomUUID(), subjectId: randomUUID(), accountId: randomUUID(), amountMicros: 25, programKey: 'support', operatorId: 'operator-1', reason: 'support adjustment', idempotencyKey: `admin-grant-${randomUUID()}` } as const;
    try {
  ...
  it('rejects an account id that belongs to a different subject', async () => {
    ...
    try {
      await connection.execute(
        ...
        [accountId, siteId, 'subject-owner'],
      );
      await expect(service.grant({ siteId, subjectId: 'subject-forged', accountId, amountMicros: 25, programKey: 'support', operatorId: 'operator-1', reason: 'support adjustment', idempotencyKey: `admin-grant-mismatch-${randomUUID()}` })).rejects.toThrow('billing.credit_account_mismatch');
    } finally {
