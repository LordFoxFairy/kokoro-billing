  ...
  it("holds by grant burn order, captures actual usage and releases the difference", async () => {
    ...
    try {
      await settlement.recordSettlement({
        ...
        externalPaymentRef: `usage-payment-${randomUUID()}`,
        amountMinor: 1000,
        currency: "USD",
      });
      ...
      await usage.recordUsageEvent({
        ...
        tenantId,
        subjectId: `subject-${accountId}`,
        sourceEventId: `usage-${randomUUID()}`,
        featureKey: "model.request",
        quantityMicros: 30,
      });
      const foreignUsageEventId = randomUUID();
      await usage.recordUsageEvent({
        usageEventId: foreignUsageEventId,
        tenantId,
        subjectId: `other-subject-${accountId}`,
        sourceEventId: `usage-${randomUUID()}`,
        featureKey: "model.request",
        quantityMicros: 30,
      });
      ...
      const [heldRows] = await connection.query<RowDataPacket[]>(
        "SELECT available_micros, held_micros FROM entitlement_credit_account WHERE credit_account_id = $1",
        [accountId],
      );
      ...
      await expect(
        usage.settleUsage({
          ...
          holdId: hold.holdId,
          usageEventId: foreignUsageEventId,
          idempotencyKey: `settle-foreign-${randomUUID()}`,
          actualMicros: 20,
        }),
      ).rejects.toThrow("billing.usage_event_mismatch");
      const first = await usage.settleUsage({
        tenantId,
        holdId: hold.holdId,
        usageEventId,
      ...
      const second = await usage.settleUsage({
        tenantId,
        holdId: hold.holdId,
        usageEventId,
        idempotencyKey: `settle-replay-${randomUUID()}`,
        actualMicros: 20,
      });
      ...
      await expect(
        usage.settleUsage({
          ...
          holdId: hold.holdId,
          usageEventId,
          idempotencyKey: `settle-conflict-${randomUUID()}`,
          actualMicros: 19,
        }),
      ).rejects.toThrow("billing.idempotency_conflict");
  ...
  it("releases an active hold idempotently without debiting the account", async () => {
    ...
    try {
      ...
      await settlement.recordSettlement({
        ...
        tenantId,
        idempotencyKey: `settlement-${settlementId}`,
        externalPaymentRef: `release-payment-${randomUUID()}`,
        amountMinor: 1000,
        currency: "USD",
      });
      await settlement.fulfillSettlement({
        ...
        accountId,
        subjectId: `subject-${accountId}`,
        programKey: "release-plan",
        grantMicros: 100,
      });
      const hold = await usage.authorizeUsage({
        tenantId,
        accountId,
        idempotencyKey: `release-hold-${randomUUID()}`,
        requestedMicros: 30,
        featureKey: "model.request",
      });
      const first = await usage.releaseUsage({
        tenantId,
        holdId: hold.holdId,
        idempotencyKey: `release-${randomUUID()}`,
      });
      const replay = await usage.releaseUsage({
        tenantId,
        holdId: hold.holdId,
        idempotencyKey: `release-replay-${randomUUID()}`,
      });
      expect(first).toEqual(replay);
      const [accountRows] = await connection.query<
        (RowDataPacket & { available_micros: number; held_micros: number })[]
      >(
        "SELECT available_micros, held_micros FROM entitlement_credit_account WHERE credit_account_id = $1",
        [accountId],
      );
  ...
  it("expires abandoned holds and returns their reservation to the account", async () => {
    ...
    try {
      ...
      await settlement.recordSettlement({
        ...
        tenantId,
        idempotencyKey: `settlement-${settlementId}`,
        externalPaymentRef: `expiry-payment-${randomUUID()}`,
        amountMinor: 1000,
        currency: "USD",
      });
      await settlement.fulfillSettlement({
        ...
        accountId,
        subjectId: `subject-${accountId}`,
        programKey: "expiry-plan",
        grantMicros: 100,
      });
      const hold = await usage.authorizeUsage({
        tenantId,
        accountId,
        idempotencyKey: `expiry-hold-${randomUUID()}`,
        requestedMicros: 30,
        featureKey: "model.request",
      });
      await connection.execute(
        `UPDATE entitlement_credit_hold SET expires_at = CURRENT_TIMESTAMP(3) - INTERVAL '1 second' WHERE credit_hold_id = $1`,
        [hold.holdId],
      );
      const batchId = `expiry-batch-${randomUUID()}`;
      const idempotencyKey = `expiry-${randomUUID()}`;
      expect(
        await usage.expireExpiredHolds({ tenantId, batchId, idempotencyKey }),
      ).toEqual({ batchId, expiredHoldIds: [hold.holdId] });
      expect(
        await usage.expireExpiredHolds({ tenantId, batchId, idempotencyKey }),
      ).toEqual({ batchId, expiredHoldIds: [hold.holdId] });
      const nextBatchId = `expiry-batch-${randomUUID()}`;
      expect(
        await usage.expireExpiredHolds({
          tenantId,
          batchId: nextBatchId,
          idempotencyKey: `expiry-${randomUUID()}`,
        }),
      ).toEqual({ batchId: nextBatchId, expiredHoldIds: [] });
  ...
  it("rejects a usage event replay with a different payload", async () => {
    ...
    try {
      await usage.recordUsageEvent({
        ...
        featureKey: "model.request",
        quantityMicros: 10,
        dimensions: { model: "MODEL", region: "us-east" },
      });
      await expect(
        usage.recordUsageEvent({
          ...
          featureKey: "model.request",
          quantityMicros: 10,
          dimensions: { model: "OTHER", region: "us-east" },
        }),
      ).rejects.toThrow("billing.idempotency_conflict");
    } finally {
      await connection.end();
