  ...
  it('quotes token usage from the active immutable revision and exposes reservation micros', async () => {
    ...
    try {
      ...
      await connection.execute(
        ...
        [rateId, revisionId, siteId],
      );
      await expect(service.quote({ siteId, featureKey: 'chat', labelKey: 'pro', inputTokens: 1000, outputTokens: 2000 })).resolves.toMatchObject({
      ...
      await connection.execute(
        ...
        [randomUUID(), revisionId, siteId],
      );
      await expect(service.quote({ siteId, featureKey: 'chat', labelKey: 'micro', inputTokens: 1, outputTokens: 0 })).resolves.toMatchObject({
        amountMicros: 1,
      });
    } finally {
      await connection.execute('DELETE FROM entitlement_usage_price_rate WHERE tenant_id = $1', [siteId]);
      await connection.execute('DELETE FROM entitlement_usage_price_revision WHERE tenant_id = $1', [siteId]);
