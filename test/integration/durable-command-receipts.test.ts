...
integration("durable Billing command receipts", () => {
  ...
  it("rejects settlement payload drift under the same key and settlement identity", async () => {
    ...
    try {
      ...
      await expect(
        settlement.recordSettlement({ ...input, amountMinor: 2_000 }),
      ).rejects.toThrow("billing.idempotency_conflict");
    } finally {
      await connection.end();
    }
  ...
  it("replays an expiry batch result without consuming the next eligible batch", async () => {
    ...
    try {
      ...
      expect(identityReplay).toEqual(first);
      await expect(
        usage.expireExpiredHolds({
          ...command,
          batchId: `expiry-next-${randomUUID()}`,
        }),
      ).rejects.toThrow("billing.idempotency_conflict");

      const [secondHolds] = await connection.query<
  ...
  it.each([
    ...
    "returns the stable $status outcome for a durable settlement receipt",
    async ({ status, expected }) => {
      ...
      try {
        ...
        await expect(settlement.recordSettlement(input)).rejects.toThrow(
          expected,
        );
      } finally {
        await connection.end();
  ...
  it.each([
    ...
    "returns the stable $status outcome for a durable expiry receipt",
    async ({ status, expected }) => {
      const connection = await createBillingConnection(
        assertDefined(databaseUrl),
      );
