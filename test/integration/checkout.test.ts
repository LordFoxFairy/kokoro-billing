...
integration("checkout quote snapshot", () => {
  it("creates one checkout and replays the same snapshot idempotently", async () => {
    ...
    try {
      ...
      await expect(
        service.create({ ...input, amountMinor: 2000 }),
  ...
  it("does not create a checkout from an expired quote", async () => {
    ...
    await expect(
      service.create({
        ...
        offerRevisionId: randomUUID(),
        amountMinor: 100,
        currency: "USD",
        quoteSnapshot: { key: "starter", creditMicros: "1000" },
  ...
  it("persists a hosted provider session and replays it without creating a second session", async () => {
    ...
    try {
      await connection.execute(
        `INSERT INTO entitlement_offer (offer_id, tenant_id, offer_key, status) VALUES ($1, $2, 'hosted', 'active')`,
        [offerId, tenantId],
      );
      await connection.execute(
        `INSERT INTO entitlement_offer_revision
        ...
        hostedProvider: {
          ...
          createSession: async (input) => {
            ...
            return Promise.resolve({
              provider: "fake",
              sessionId: `session-${input.checkoutId}`,
      ...
      const checkout = await service.create({
        ...
        amountMinor: 1200,
        currency: "USD",
        quoteSnapshot: { key: "hosted", creditMicros: "9000", name: "Hosted" },
        expiresAt: new Date(Date.now() + 60_000),
      });
      const first = await service.createHostedSession(
        tenantId,
        checkout.checkoutId,
      );
      const second = await service.createHostedSession(
        tenantId,
        checkout.checkoutId,
      );
      expect(first.checkoutUrl).toBe(
        "https://provider.example/checkout/session",
      );
