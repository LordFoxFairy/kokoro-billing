...
integration('payment reversal to credit reversal', () => {
  it('reverses only the unconsumed grant amount exactly once', async () => {
    ...
    try {
      ...
      const [journalRows] = await connection.query('SELECT journal_id FROM entitlement_credit_journal WHERE source_ref = $1', [reversalId]);
      const [outboxRows] = await connection.query('SELECT outbox_id FROM payment_outbox WHERE aggregate_type = $1 AND aggregate_id = $2 AND event_type = $3', ['payment_reversal', reversalId, 'PaymentReversalRecorded']);
