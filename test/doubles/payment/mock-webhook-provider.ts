...
} from "../../../src/application/payment/ports/provider-types.js";
...
export class MockWebhookProvider implements PaymentWebhookProvider {
  ...
  public parseEvent(payload: unknown): ParsedWebhookEvent {
    ...
    return {
      ...
      payloadTenantId: parsed.data?.tenantId ?? null,
      providerAccountRef: null,
      externalPaymentRef:
        parsed.eventType === "payment_succeeded" ? parsed.eventId : null,
      externalReversalRef:
        parsed.eventType === "refund_succeeded" ? parsed.eventId : null,
      refundAmountMinor:
        typeof parsed.data?.refundAmountMinor === "number"
          ? parsed.data.refundAmountMinor
          : null,
