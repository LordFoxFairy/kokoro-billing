...
const state = vi.hoisted(() => {
  const value: { failure: unknown; closeError: Error | undefined } = {
    failure: undefined, closeError: undefined,
  };
  ...
  Pool: class {
    ...
    connect() {
      return Promise.resolve({
        on: vi.fn(), removeListener: vi.fn(), release: state.release,
        query: () => Promise.resolve().then(() => {
      ...
      state.close();
      return state.value.closeError === undefined ? Promise.resolve() : Promise.reject(state.value.closeError);
    }
  },
}));
...
describe('canonical installer error identity (unit fault injection)', () => {
  it.each([undefined, null, false, 0, '', new Error('original installation error')])(
    ...
    async (failure) => {
      ...
      for (const closeError of [undefined, new Error('close failed')]) {
        ...
        const installing = installCanonicalSchema({ databaseUrl: 'postgresql://fixture@localhost/fixture', schemaSql: 'SELECT 1;' });
