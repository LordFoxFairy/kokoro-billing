...
describe('Billing state machines', () => {
  it('permits only forward payment and admission transitions', () => {
    expect(canTransition('paymentCollection', 'authorized', 'captured')).toBe(true);
    expect(canTransition('paymentCollection', 'captured', 'authorized')).toBe(false);
  ...
  it('rejects a terminal-state transition', () => {
    expect(() => assertTransition('admission', 'released', 'captured')).toThrow('billing.invalid_admission_transition');
  });
});
