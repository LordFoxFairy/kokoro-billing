...
import { z } from 'zod';
import { PersistedDataInvariantError, parsePersistedJson } from '../../src/infrastructure/postgres/json.js';
...
describe('persisted JSON invariants', () => {
  it('keeps the internal invariant code while exposing only a generic 500', () => {
    let thrown: unknown;
    try {
      parsePersistedJson({ accepted: false }, z.object({ accepted: z.literal(true) }), 'billing.command_result_invalid');
    } catch (error) {
      thrown = error;
    }
