...
describe('payment provider registry', () => {
  it('reuses the mature Stripe adapter only when explicitly enabled', () => {
    ...
    const rawBody = Buffer.from(JSON.stringify({ id: 'evt-1', type: 'unknown', data: {} }));
    const timestamp = Math.floor(Date.now() / 1000);
    const signature = signStripeWebhook(rawBody, 'secret', timestamp);
    expect(verifyProviderWebhook(registry, 'stripe', { 'stripe-signature': signature }, rawBody, 'secret')).toBe(true);
    expect(verifyProviderWebhook(registry, 'wechat', {}, rawBody, 'secret')).toBe(false);
  });
  ...
  it('supports the local mock HMAC adapter only when explicitly enabled', () => {
    ...
    expect(parseProviderWebhook(registry, 'mock', JSON.parse(rawBody.toString()))).toMatchObject({ eventId: 'mock-1', orderId: 'checkout-1' });
  });

  it('accepts Alipay body-signature verification without a synthetic signature header', () => {
    const { privateKey, publicKey } = generateKeyPairSync('rsa', { modulusLength: 2048 });
    const params = { notify_id: 'notify-1', sign_type: 'RSA2', app_id: 'app-1', trade_status: 'TRADE_SUCCESS', trade_no: 'trade-1' };
    ...
    expect(verifyProviderWebhook(registry, 'alipay', {}, rawBody, publicKeyPem)).toBe(true);
  ...
  it('decrypts WeChat APIv3 encrypted resources before normalizing the order reference', () => {
    ...
    const ciphertext = Buffer.concat([cipher.update(JSON.stringify({ out_trade_no: 'checkout-wechat-1' })), cipher.final(), cipher.getAuthTag()]).toString('base64');
    const provider = new WechatWebhookProvider({ apiV3Key: key.toString('utf8') });
    expect(provider.parseEvent({ id: 'wx-event-1', event_type: 'TRANSACTION.SUCCESS', resource: { ciphertext, nonce, associated_data: associatedData } })).toMatchObject({ orderId: 'checkout-wechat-1' });
  });
});
