...
import { assertProviderWebhookSecrets, assertWechatApiV3Key, readEnabledProviders, readProviderWebhookSecrets } from '../../src/config/provider-config.js';

describe('payment provider runtime configuration', () => {
  it('normalizes enabled providers and rejects unknown values', () => {
    expect(readEnabledProviders(' stripe, wechat ')).toEqual(['stripe', 'wechat']);
    expect(() => readEnabledProviders('stripe,typo')).toThrow('unsupported BILLING_ENABLED_PROVIDERS: typo');
  });

  it('requires a secret for every enabled webhook provider', () => {
    const secrets = readProviderWebhookSecrets('{"stripe":"secret"}');
    expect(() => assertProviderWebhookSecrets(['stripe'], secrets)).not.toThrow();
    expect(() => assertProviderWebhookSecrets(['wechat'], secrets)).toThrow('PROVIDER_WEBHOOK_SECRETS_JSON.wechat is required');
