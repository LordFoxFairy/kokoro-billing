...
`database/schema.sql` is the sole canonical schema. B8-M2b currently defines 32 `billing_*` resources with application-generated UUID primary keys and no foreign keys. The added command-key binding normalizes permanent key-to-receipt facts. This remains an offline, non-deployable intermediate until the existing pg writers are switched as one transaction group; no legacy tables or compatibility views are retained.
...
REPEATABLE READ target snapshot. It compares all 32 relations and their complete generated columns, constraints and indexes,
