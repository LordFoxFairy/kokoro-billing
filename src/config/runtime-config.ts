...
const readPort = (env: NodeJS.ProcessEnv): number => {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535)
    throw new Error("BILLING_PORT must be an integer between 1 and 65535");
  return port;
};

const readPositiveMilliseconds = (
...
const readTimeouts = (
  ...
  const timeouts = {
    connectTimeoutMs: readPositiveMilliseconds(
      env,
      `${prefix}_CONNECT_TIMEOUT_MS`,
      defaults.connectTimeoutMs,
    ),
    readTimeoutMs: readPositiveMilliseconds(
      env,
      `${prefix}_READ_TIMEOUT_MS`,
      defaults.readTimeoutMs,
    ),
    overallTimeoutMs: readPositiveMilliseconds(
      env,
      `${prefix}_OVERALL_TIMEOUT_MS`,
      defaults.overallTimeoutMs,
    ),
  };
  if (
    timeouts.overallTimeoutMs <
    Math.max(timeouts.connectTimeoutMs, timeouts.readTimeoutMs)
  ) {
    throw new Error(
      `${prefix}_OVERALL_TIMEOUT_MS must be at least the connect and read timeouts`,
    );
  }
  return timeouts;
};
...
const readAuthMode = (env: NodeJS.ProcessEnv): BillingAuthMode => {
  ...
  if (raw !== "internal-header" && raw !== "jwks")
    throw new Error("BILLING_AUTH_MODE must be internal-header or jwks");
  if (env.NODE_ENV === "production" && raw !== "jwks")
    throw new Error("BILLING_AUTH_MODE=jwks is required in production");
  return raw;
};

export const readBillingRuntimeConfig = (
  ...
  if (authMode === "jwks" && (jwksUrl === undefined || jwksUrl.length === 0))
    throw new Error("BILLING_AUTH_JWKS_URL is required");

  const enabledProviders = readEnabledProviders(
    env.BILLING_ENABLED_PROVIDERS ?? "",
  );
  const providerSecrets = readProviderWebhookSecrets(
    env.PROVIDER_WEBHOOK_SECRETS_JSON ?? "{}",
  );
  ...
  const redisTimeouts = readTimeouts(env, "REDIS", {
    connectTimeoutMs: 2_000,
    readTimeoutMs: 1_000,
    overallTimeoutMs: 3_000,
  });
  const stripeTimeouts = readTimeouts(env, "STRIPE", {
  ...
  for (const provider of ALL_PAYMENT_PROVIDERS) {
    providerAccountRefs[provider] =
      env[`BILLING_PROVIDER_ACCOUNT_REF_${provider.toUpperCase()}`] ?? null;
  }
  ...
  return {
    ...
    ...(env.BILLING_AUTH_JWT_AUDIENCE === undefined
      ? {}
      : { audience: env.BILLING_AUTH_JWT_AUDIENCE }),
    enabledProviders,
    providerSecrets,
    ...
    ...(providerAccountRefs.stripe === null
      ? {}
      : { stripeAccountRef: providerAccountRefs.stripe }),
    providerAccountRefs,
    publicBaseUrl: env.BILLING_PUBLIC_BASE_URL ?? "http://127.0.0.1:3000",
    redisTimeouts,
    stripeTimeouts,
    shutdownDeadlineMs: readPositiveMilliseconds(
      env,
      "BILLING_SHUTDOWN_DEADLINE_MS",
      10_000,
    ),
