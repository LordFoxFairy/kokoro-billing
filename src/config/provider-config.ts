...
const PROVIDER_KINDS = ['stripe', 'alipay', 'wechat'] as const satisfies readonly PaymentProviderKind[];
...
export function readEnabledProviders(raw = process.env.BILLING_ENABLED_PROVIDERS ?? ''): PaymentProviderKind[] {
  ...
  if (!result.success) {
    const unsupported = configured.filter((value) => !paymentProviderKindSchema.safeParse(value).success);
    throw new Error(`unsupported BILLING_ENABLED_PROVIDERS: ${unsupported.join(',')}`);
  }
  return result.data;
}
...
export function readProviderWebhookSecrets(raw = process.env.PROVIDER_WEBHOOK_SECRETS_JSON ?? '{}'): Record<string, string> {
  ...
  if (!result.success) throw new Error('PROVIDER_WEBHOOK_SECRETS_JSON must be a JSON object of non-empty strings');
  return result.data;
}
