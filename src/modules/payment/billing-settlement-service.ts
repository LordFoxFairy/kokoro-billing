  ...
  public async recordSettlement(input: RecordSettlementInput): Promise<void> {
    ...
    try {
      await this.connection.execute(
        ...
        [input.settlementId, input.siteId, input.providerEventId ?? null, input.checkoutId ?? null, provider, input.externalPaymentRef, input.amountMinor, input.currency],
      );
      const [rows] = await this.connection.execute<SettlementRow[]>(
        ...
        [input.siteId, provider, input.externalPaymentRef],
      );
      const row = rows[0];
      if (!row) throw new Error('billing.settlement_not_found');
      if (row.settlement_id !== input.settlementId || row.tenant_id !== input.siteId || row.provider !== provider || row.external_payment_ref !== input.externalPaymentRef || String(row.amount_minor) !== String(input.amountMinor) || row.currency !== input.currency) {
      ...
      await this.connection.execute(
        `INSERT INTO payment_outbox
         ...
         ON CONFLICT DO NOTHING`,
        [randomUUID(), input.siteId, input.settlementId, JSON.stringify({ settlementId: input.settlementId, provider, externalPaymentRef: input.externalPaymentRef, amountMinor: input.amountMinor, currency: input.currency })],
      );
  ...
  public async fulfillSettlement(input: FulfillSettlementInput): Promise<FulfillmentResult> {
    ...
    try {
      const [settlements] = await this.connection.execute<SettlementRow[]>(
        ...
        [input.settlementId, input.siteId],
      );
      ...
      const [existing] = await this.connection.execute<FulfillmentRow[]>(
        `SELECT f.fulfillment_id, g.credit_grant_id, j.journal_id,
           ...
           JOIN entitlement_acquisition a ON a.acquisition_id = f.acquisition_id
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_acquisition
         ...
         ON CONFLICT DO NOTHING`,
        [acquisitionId, input.siteId, input.subjectId, input.settlementId, input.programKey, input.grantMicros],
      );
      const [acquisitions] = await this.connection.execute<RowDataPacket[]>(
        ...
        [input.siteId, input.settlementId, input.programKey],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_grant
         ...
         VALUES ($1, $2, $3, 'payment_settlement', $4, $5, $6, $7, CURRENT_TIMESTAMP(6))
         ON CONFLICT DO NOTHING`,
        [grantId, input.siteId, input.accountId, input.settlementId, input.programKey, input.grantMicros, input.grantMicros],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_journal
         ...
         ON CONFLICT DO NOTHING`,
        [journalId, input.siteId, input.accountId, nextSequence, input.grantMicros, input.settlementId],
      );
      ...
      const [accountUpdate] = await this.connection.execute<ResultSetHeader>(
        ...
        [input.grantMicros, input.siteId, input.accountId],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_outbox
         ...
         ON CONFLICT DO NOTHING`,
        [randomUUID(), input.siteId, fulfillment.fulfillment_id, JSON.stringify({ settlementId: input.settlementId, grantId: grant.credit_grant_id })],
      );
      await this.connection.commit();
