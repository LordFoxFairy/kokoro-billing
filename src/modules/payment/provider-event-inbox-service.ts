...
type ProviderEventRow = RowDataPacket & { provider_event_id: string; provider_account_ref: string | null; processing_status: 'received' | 'processed' | 'ignored' | 'failed'; payload_hash: string; event_type: string };
...
export class ProviderEventInboxService {
  ...
  public async accept(input: ProviderEventInput): Promise<ProviderEventFact> {
    ...
    try {
      ...
      if (row.payload_hash !== payloadHash || row.event_type !== input.eventType || row.provider_account_ref !== (input.providerAccountRef ?? null)) throw new Error('billing.idempotency_conflict');
      await this.connection.execute(
