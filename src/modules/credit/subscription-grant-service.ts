...
export type CreditGrantResult = { readonly grantId: string; readonly journalId: string };

/** Idempotent period -> acquisition -> fulfillment -> grant -> journal projection. */
export class SubscriptionGrantService {
  public constructor(private readonly connection: Connection) {}

  public async grant(input: SubscriptionGrantInput): Promise<CreditGrantResult> {
    if (!Number.isSafeInteger(input.amountMicros) || input.amountMicros <= 0) throw new RangeError('amountMicros must be positive');
    if (Number.isNaN(input.expiresAt.getTime()) || input.expiresAt.getTime() <= Date.now()) throw new Error('billing.subscription_period_expired');
    await this.connection.beginTransaction();
    try {
      await this.connection.execute(
        `INSERT INTO entitlement_credit_account (credit_account_id, tenant_id, subject_id)
         VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
        [input.accountId, input.siteId, input.subjectId],
      );
      const [accounts] = await this.connection.execute<(RowDataPacket & { credit_account_id: string })[]>(
      ...
      if (accounts[0]?.credit_account_id !== input.accountId) throw new Error('billing.credit_account_mismatch');
      ...
      await this.connection.execute(
        ...
        [acquisitionId, input.siteId, input.subjectId, input.periodId, input.programKey, input.amountMicros],
      );
      const [acquisitions] = await this.connection.execute<(RowDataPacket & { acquisition_id: string })[]>(
        ...
        [input.siteId, input.periodId, input.programKey],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_fulfillment (fulfillment_id, tenant_id, acquisition_id, status, committed_at)
         VALUES ($1, $2, $3, 'committed', CURRENT_TIMESTAMP(6))
         ON CONFLICT DO NOTHING`,
        [fulfillmentId, input.siteId, acquisition.acquisition_id],
      );
      const [fulfillments] = await this.connection.execute<(RowDataPacket & { fulfillment_id: string })[]>(
        `SELECT fulfillment_id FROM entitlement_fulfillment WHERE tenant_id = $1 AND acquisition_id = $2 FOR UPDATE`,
        [input.siteId, acquisition.acquisition_id],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_grant
         ...
         VALUES ($1, $2, $3, 'subscription_period', $4, $5, $6, $7, CURRENT_TIMESTAMP(6), $8)
         ON CONFLICT DO NOTHING`,
        [grantId, input.siteId, input.accountId, input.periodId, input.programKey, input.amountMicros, input.amountMicros, input.expiresAt],
      );
      const [grants] = await this.connection.execute<(RowDataPacket & { credit_grant_id: string; original_micros: number | string })[]>(
        `SELECT credit_grant_id, original_micros, expires_at FROM entitlement_credit_grant
          WHERE tenant_id = $1 AND source_kind = 'subscription_period' AND source_ref = $2 AND program_key = $3 FOR UPDATE`,
        [input.siteId, input.periodId, input.programKey],
      );
      const grant = grants[0] as (RowDataPacket & { credit_grant_id: string; original_micros: number | string; expires_at: Date | string | null }) | undefined;
      if (!grant || String(grant.original_micros) !== String(input.amountMicros) || grant.expires_at === null || new Date(grant.expires_at).getTime() !== input.expiresAt.getTime()) throw new Error('billing.idempotency_conflict');

      const [priorJournals] = await this.connection.execute<(RowDataPacket & { journal_id: string })[]>(
        ...
        [input.siteId, input.periodId],
      );
      if (priorJournals[0]) {
        await this.connection.commit();
        return { grantId: grant.credit_grant_id, journalId: priorJournals[0].journal_id };
      }

      const [sequences] = await this.connection.execute<(RowDataPacket & { journal_seq: number | string })[]>(
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_journal
         ...
         ON CONFLICT DO NOTHING`,
        [journalId, input.siteId, input.accountId, (BigInt(String(sequences[0]?.journal_seq ?? 0)) + 1n).toString(), input.amountMicros, input.periodId],
      );
      const [journals] = await this.connection.execute<(RowDataPacket & { journal_id: string })[]>(
        `SELECT journal_id FROM entitlement_credit_journal
          WHERE tenant_id = $1 AND source_kind = 'subscription_period' AND source_ref = $2 AND entry_kind = 'grant' FOR UPDATE`,
        [input.siteId, input.periodId],
      );
      ...
      const [updated] = await this.connection.execute<ResultSetHeader>(
        ...
        [input.amountMicros, input.siteId, input.accountId],
      );
      if (updated.affectedRows !== 1) throw new Error('billing.credit_projection_drift');
      await this.connection.execute(
        `INSERT INTO entitlement_outbox (outbox_id, tenant_id, aggregate_type, aggregate_id, event_type, payload_json)
         ...
         ON CONFLICT DO NOTHING`,
        [randomUUID(), input.siteId, grant.credit_grant_id, JSON.stringify({ periodId: input.periodId, grantId: grant.credit_grant_id })],
      );
      await this.connection.commit();
