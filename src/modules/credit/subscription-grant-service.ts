  ...
  public async grant(input: SubscriptionGrantInput): Promise<CreditGrantResult> {
    ...
    try {
      ...
      await this.connection.execute(
        ...
        [acquisitionId, input.tenantId, input.subjectId, input.periodId, input.programKey, input.amountMicros],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_grant
         ...
         ON CONFLICT DO NOTHING`,
        [grantId, input.tenantId, input.accountId, input.periodId, input.programKey, input.amountMicros, input.amountMicros, input.expiresAt],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_journal
         ...
         ON CONFLICT DO NOTHING`,
        [journalId, input.tenantId, input.accountId, (BigInt(String(sequences[0]?.journal_seq ?? 0)) + 1n).toString(), input.amountMicros, input.periodId],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_outbox (outbox_id, tenant_id, aggregate_type, aggregate_id, event_type, payload_json)
         ...
         ON CONFLICT DO NOTHING`,
        [randomUUID(), input.tenantId, grant.credit_grant_id, JSON.stringify({ periodId: input.periodId, grantId: grant.credit_grant_id })],
      );
      await this.connection.commit();
