  ...
  public async expireExpiredGrants(input: { readonly tenantId?: string; readonly limit?: number } = {}): Promise<GrantExpiryResult> {
    ...
    const [grants] = await this.connection.query<RowDataPacket[]>(
      `SELECT g.credit_grant_id, g.tenant_id
        ...
        WHERE g.status = 'active' AND g.remaining_micros > 0
          AND g.expires_at IS NOT NULL AND g.expires_at <= CURRENT_TIMESTAMP(6)
          AND NOT EXISTS (
            SELECT 1
    ...
    for (const row of grants) {
      ...
      try {
        ...
        const grant = lockedRows[0] as { credit_grant_id: string; tenant_id: string; credit_account_id: string; remaining_micros: string | number; status: string } | undefined;
        if (!grant) { await this.connection.rollback(); continue; }
        const [activeAllocations] = await this.connection.execute<RowDataPacket[]>(
          `SELECT 1
             FROM entitlement_credit_hold_allocation ha
             INNER JOIN entitlement_credit_hold h ON h.credit_hold_id = ha.credit_hold_id
        ...
        if (activeAllocations[0]) { await this.connection.rollback(); continue; }
        const remaining = readSafeInteger(grant.remaining_micros, 'grant_remaining_micros');
        if (remaining > 0) {
          ...
          await this.connection.execute(
            ...
            [randomUUID(), grant.tenant_id, grant.credit_account_id, journalSeq, -remaining, grantId],
          );
        ...
        await this.connection.execute(
          ...
          [randomUUID(), grant.tenant_id, grantId, JSON.stringify({ grantId, reason: 'expires_at' })],
        );
