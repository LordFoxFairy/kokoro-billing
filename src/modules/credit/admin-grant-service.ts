  ...
  public async grant(input: AdminGrantInput): Promise<AdminGrantResult> {
    ...
    try {
      ...
      if (prior[0]) {
        ...
        const result = (typeof stored === 'string' ? JSON.parse(stored) : stored) as AdminGrantResult;
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_credit_grant
         ...
         VALUES ($1, $2, $3, 'admin_grant', $4, $5, $6, $7, CURRENT_TIMESTAMP(6))`,
        [grantId, input.siteId, input.accountId, receiptId, input.programKey, input.amountMicros, input.amountMicros],
      );
      ...
      await this.connection.execute(
        ...
        [journalId, input.siteId, input.accountId, (BigInt(String((sequences[0] as { journal_seq: number | string }).journal_seq)) + 1n).toString(), input.amountMicros, receiptId],
      );
      ...
      await this.connection.execute(
        ...
        [randomUUID(), input.siteId, input.operatorId, input.subjectId, grantId, input.reason, JSON.stringify({ amountMicros: input.amountMicros, programKey: input.programKey, idempotencyKey: input.idempotencyKey })],
      );
      await this.connection.execute(
        ...
        [randomUUID(), input.siteId, grantId, JSON.stringify({ grantId, operatorId: input.operatorId, reason: input.reason })],
      );
      await this.connection.commit();
      return result;
    } catch (error) {
