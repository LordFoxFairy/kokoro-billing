...
export class RedeemService {
  ...
  public async redeem(input: RedeemInput): Promise<RedeemResult> {
    ...
    try {
      ...
      const [codeUpdate] = await this.connection.execute<ResultSetHeader>(`UPDATE entitlement_redeem_code SET status = 'redeemed', redeemed_by = $1, redeemed_at = CURRENT_TIMESTAMP(6) WHERE tenant_id = $2 AND code_id = $3 AND status = 'issued'`, [input.subjectId, input.tenantId, code.code_id]);
      ...
      await this.connection.execute(`INSERT INTO entitlement_credit_grant (credit_grant_id, tenant_id, credit_account_id, source_kind, source_ref, program_key, original_micros, remaining_micros, effective_at) VALUES ($1, $2, $3, 'redeem', $4, $5, $6, $7, CURRENT_TIMESTAMP(6))`, [grantId, input.tenantId, accountId, redemptionId, code.program_key, amount, amount]);
