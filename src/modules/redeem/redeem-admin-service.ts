...
export class RedeemAdminService {
  ...
  public async createCampaign(input: CreateCampaignInput): Promise<CampaignResult> {
    ...
    return this.withReceipt('RedeemCreateCampaign', input.tenantId, input.idempotencyKey, { ...input, idempotencyKey: undefined }, async () => {
  ...
  public async issueCodes(input: IssueCodesInput): Promise<IssueCodesResult> {
    ...
    return this.withReceipt('RedeemIssueCodes', input.tenantId, input.idempotencyKey, { ...input, idempotencyKey: undefined }, async () => {
  ...
  public async disableCode(input: { readonly tenantId: string; readonly codeId: string; readonly operatorId: string; readonly reason: string; readonly idempotencyKey: string }): Promise<void> {
    await this.withReceipt('RedeemDisableCode', input.tenantId, input.idempotencyKey, { ...input, idempotencyKey: undefined }, async () => {
  ...
  private async withReceipt<T>(command: string, tenantId: string, key: string, payload: object, work: () => Promise<T>, persist: (result: T) => T = (result) => result): Promise<T> {
