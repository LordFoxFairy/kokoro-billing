  ...
  public async publishPlan(input: PublishCatalogPlanInput): Promise<CatalogPlan> {
    ...
    try {
      ...
      if (receipt.status === 'succeeded' && receipt.result_json !== null) {
        ...
        return (typeof receipt.result_json === 'string' ? JSON.parse(receipt.result_json) : receipt.result_json) as CatalogPlan;
      }
      ...
      await this.connection.execute(
        ...
        [randomUUID(), input.siteId, input.offerKey],
      );
      ...
      await this.connection.execute(
        `INSERT INTO entitlement_offer_revision
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'published', CURRENT_TIMESTAMP(6))`,
        [revisionId, offerId, input.siteId, revision, input.name, input.currency, input.amountMinor, input.creditMicros, input.billingInterval],
      );
      await this.connection.execute(
        ...
        [randomUUID(), input.siteId, input.operatorId, revisionId, input.reason, input.offerKey, revision],
      );
      await this.connection.commit();
      return result;
    } catch (error) {
