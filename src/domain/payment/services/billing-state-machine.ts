...
export type AdmissionStatus =
  "created" | "held" | "captured" | "released" | "unknown" | "rejected";
...
const transitions = {
  paymentCollection: {
    ...
    failed: [],
    cancelled: [],
    unknown: ["authorized", "captured", "failed", "cancelled", "unknown"],
  } satisfies Record<
    PaymentCollectionStatus,
    readonly PaymentCollectionStatus[]
  >,
  paymentAttempt: {
  ...
  admission: {
    created: ["held", "rejected", "unknown"],
    held: ["captured", "released", "unknown"],
    captured: [],
    released: [],
    unknown: ["captured", "released", "unknown"],
    rejected: [],
  } satisfies Record<AdmissionStatus, readonly AdmissionStatus[]>,
} as const;

export function canTransition<K extends keyof typeof transitions>(
  kind: K,
  from: keyof (typeof transitions)[K],
  to: string,
): boolean {
  return (transitions[kind][from] as readonly string[]).includes(to);
}

export function assertTransition<K extends keyof typeof transitions>(
  kind: K,
  from: keyof (typeof transitions)[K],
  to: string,
): void {
  if (!canTransition(kind, from, to))
    throw new Error(`billing.invalid_${String(kind)}_transition`);
}
