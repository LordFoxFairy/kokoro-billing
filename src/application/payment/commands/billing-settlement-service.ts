...
export type RecordSettlementInput = { readonly settlementId: string; readonly tenantId: string; readonly idempotencyKey: string; readonly externalPaymentRef: string; readonly amountMinor: number; readonly currency: string; readonly provider?: string; readonly providerEventId?: string; readonly checkoutId?: string };
export type SettlementAcceptanceResult = { readonly settlementId: string; readonly accepted: true };
...
export class BillingSettlementService {
  ...
  public recordSettlement(input: RecordSettlementInput): Promise<SettlementAcceptanceResult> { return this.transaction.withTransaction(() => this.repository.recordSettlement(input)); }
