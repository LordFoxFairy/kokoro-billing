...
export type Checkout = {
  ...
  readonly status:
    "created" | "pending_payment" | "paid" | "expired" | "cancelled";
  readonly amountMinor: number;
  readonly currency: string;
  readonly expiresAt: Date;
...
export class CheckoutService {
  ...
  public create(input: CreateCheckoutInput): Promise<Checkout> {
    return this.transaction.withTransaction(() =>
      this.repository.create(input),
    );
  }
  public createHostedSession(
    tenantId: string,
    checkoutId: string,
  ): Promise<Checkout> {
    return this.transaction.withTransaction(() =>
      this.repository.createHostedSession(tenantId, checkoutId),
    );
  }
}
