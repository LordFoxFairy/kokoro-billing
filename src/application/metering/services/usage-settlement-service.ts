...
export class UsageSettlementService {
  public constructor(
    private readonly repository: UsageSettlementRepository,
    private readonly transaction: TransactionPort,
  ) {}
  public recordUsageEvent(input: UsageEventInput): Promise<void> {
    return this.transaction.withTransaction(() =>
      this.repository.recordUsageEvent(input),
    );
  }
  public ensureUsageEventForHold(input: {
    tenantId: string;
    holdId: string;
    sourceEventId: string;
  }): Promise<string> {
    return this.transaction.withTransaction(() =>
      this.repository.ensureUsageEventForHold(input),
    );
  }
  public authorizeUsage(input: AuthorizeUsageInput): Promise<UsageHold> {
    return this.transaction.withTransaction(() =>
      this.repository.authorizeUsage(input),
    );
  }
  public settleUsage(input: SettleUsageInput): Promise<UsageSettlementResult> {
    return this.transaction.withTransaction(() =>
      this.repository.settleUsage(input),
    );
  }
  public releaseUsage(input: {
    readonly tenantId: string;
    readonly holdId: string;
    readonly idempotencyKey: string;
  }): Promise<UsageReleaseResult> {
    return this.transaction.withTransaction(() =>
      this.repository.releaseUsage(input),
    );
  }
  public expireExpiredHolds(
    input: ExpireUsageHoldsInput,
  ): Promise<UsageExpiryResult> {
    return this.transaction.withTransaction(() =>
      this.repository.expireExpiredHolds(input),
    );
  }
}
