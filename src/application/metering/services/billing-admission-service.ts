...
export type BillingSubject = {
  readonly kind: "user" | "project" | "organization" | "service";
  readonly ref: string;
};
...
export class BillingAdmissionService {
  public constructor(
    private readonly repository: BillingAdmissionRepository,
  ...
  public capture(
    ...
    return this.transaction.withTransaction(() =>
      this.repository.capture(tenantId, admissionId, receipt, idempotencyKey),
    );
  }
  public release(
    ...
    return this.transaction.withTransaction(() =>
      this.repository.release(input),
    );
  }
  public recordExecutionEvent(input: ExecutionEventInput): Promise<{
    readonly eventId: string;
    readonly status: "received" | "processed";
  }> {
    return this.transaction.withTransaction(() =>
      this.repository.recordExecutionEvent(input),
    );
  }
  public processExecutionEvent(
    tenantId: string,
    eventId: string,
  ): Promise<void> {
    return this.transaction.withTransaction(() =>
      this.repository.processExecutionEvent(tenantId, eventId),
    );
  }
}
