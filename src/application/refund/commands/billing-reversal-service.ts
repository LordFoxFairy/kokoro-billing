...
export class BillingReversalService {
  public constructor(
    private readonly repository: BillingReversalRepository,
    private readonly transaction: TransactionPort,
  ) {}
  public recordReversal(input: RecordReversalInput): Promise<string> {
    return this.transaction.withTransaction(() =>
      this.repository.recordReversal(input),
    );
  }
  public reverseCredits(input: ReverseCreditsInput): Promise<ReversalResult> {
    return this.transaction.withTransaction(() =>
      this.repository.reverseCredits(input),
    );
  }
}
