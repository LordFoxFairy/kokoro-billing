...
type AdmissionPort = Pick<
  BillingAdmissionService,
  "create" | "capture" | "release" | "recordExecutionEvent"
>;
...
const decimalString = z
  ...
  .refine(
    ...
    "decimal value exceeds JavaScript safe integer range",
  );
const positiveDecimalString = decimalString.refine(
  (value) => BigInt(value) > 0n,
  "value must be positive",
);
const safeDecimal = (value: string, field: string): number =>
  readSafeInteger(value, field);
const pageQuerySchema = z
  ...
  .strict();
const billingSubjectSchema = z
  .object({
    kind: z.enum(["user", "project", "organization", "service"]),
...
const admissionSchema = z
  .object({
    ...
    payer_ref: z.string().min(1).max(255),
    feature_key: z.string().min(1).max(128),
    surface: z.string().min(1).max(32),
    invocation_id: z.string().min(1).max(255),
    execution_id: z.string().min(1).max(255),
    meter_kind: z.enum(["model_invocation", "feature", "studio_job"]),
    requested_model_tier: z.string().min(1).max(128).optional(),
  })
  .strict();
const acceptedReceiptSchema = z
  .object({
    invocation_id: z.string().min(1).max(255),
    execution_id: z.string().min(1).max(255),
    accepted_provider_ref: z.string().min(1).max(255),
    accepted_at: z.string().datetime({ offset: true }),
    service_receipt: z.record(z.string(), z.unknown()),
    receipt_schema_version: z.string().min(1).max(32),
  })
  .strict();
const releaseAdmissionSchema = z
  .object({
    invocation_id: z.string().min(1).max(255),
...
const targetSettlementSchema = z
  .object({
    settlement_id: z.string().min(1).max(36),
    external_payment_ref: z.string().min(1).max(255),
    amount_minor: positiveDecimalString,
    currency: z.string().regex(/^[A-Z]{3}$/u),
    provider: paymentProviderSchema,
  })
  .strict();
const expireCreditHoldsSchema = z
  .object({
    batch_id: z.string().min(1).max(128),
    limit: z.number().int().positive().max(500).optional(),
  })
  .strict();
const targetRefundSchema = z
  .object({
    settlement_id: z.string().min(1),
    external_ref: z.string().min(1),
    amount_minor: positiveDecimalString,
    allocation_mode: z.enum(["proportional", "line_specific"]),
...
export const classifyBillingError = (
  error: unknown,
    ...
    error instanceof WebhookError
      ...
      : error !== null &&
        ...
        ? {
            ...
            statusCode: error.statusCode,
            internalCode:
              "internalCode" in error && typeof error.internalCode === "string"
                ? error.internalCode
                : error.code,
          }
        : null;
  const rawMessage =
    error instanceof Error ? error.message : "billing.internal_error";
  const code =
    structured?.code ??
    (rawMessage.startsWith("billing.") ? rawMessage : "billing.internal_error");
  const status =
    structured?.statusCode !== undefined &&
    structured.statusCode >= 400 &&
    structured.statusCode < 600
      ...
      : code === "billing.internal_error"
        ...
        : code === "billing.insufficient_credit"
          ...
          : [
                ...
                "billing.command_unknown",
                "billing.reversal_exposure",
                "billing.credit_projection_drift",
                "billing.usage_event_mismatch",
              ].includes(code)
            ? 409
            : 400;
...
const sendError = (reply: FastifyReply, error: unknown) => {
  ...
  if (classified.statusCode >= 500) {
    reply.log.error(
      { err: error, internal_error_code: classified.internalCode },
      "billing request failed",
    );
  }
  return reply.code(classified.statusCode).send({
    error: {
      code: classified.externalCode,
      message: classified.externalMessage,
    },
  });
};
...
const requireIdempotency = (
  ...
  if (
    ...
    void reply.code(400).send({
      error: {
        code: "billing.idempotency_invalid",
        message: "Idempotency-Key must be 8-128 printable characters",
      },
    });
    return null;
  }
...
const hasStorefrontServiceMarker = (request: FastifyRequest): boolean => {
  return (
    ...
    request.headers["x-kokoro-internal-secret"] !== undefined
  );
};
...
const storefrontContext = async (
  ...
  if (serviceMarker) {
    void reply.code(403).send({
      error: {
        code: "billing.service_auth_failed",
        message: "web BFF service authentication failed",
      },
    });
    return null;
  }
  ...
  void reply.code(401).send({
    error: {
      ...
      message: "user or web BFF context required",
    },
  });
  return null;
};
...
export const createBillingServer = (
  ...
  const app = Fastify({
    ...
    genReqId: (request) => {
      ...
      return typeof incoming === "string" &&
        /^[\x20-\x7E]{1,128}$/u.test(incoming)
        ? incoming
        : randomUUID();
    },
  });
  app.addHook("onRequest", (_request, _reply, done) => {
    runWithBillingContext(() => done());
  });
  app.addHook("onRequest", async (request) => {
    request.billingStartedAt = process.hrtime.bigint();
  ...
  app.addHook("onResponse", async (request, reply) => {
    ...
    const durationSeconds =
      request.billingStartedAt === undefined
        ? 0
        : Number(process.hrtime.bigint() - request.billingStartedAt) / 1e9;
    recordHttpRequest({
    ...
    const incomingTraceId =
      request.headers["x-kokoro-trace-id"] ?? request.headers.traceparent;
    const traceId =
      typeof incomingTraceId === "string" &&
      /^[\x20-\x7E]{1,256}$/u.test(incomingTraceId)
        ? incomingTraceId
        : request.id;
    request.log.info(
      {
  ...
  app.get("/healthz", async (_request, reply) =>
    reply.code(200).send({ data: { module: "kokoro-billing", status: "ok" } }),
  );
  app.get("/readyz", async (_request, reply) => {
    if (!dependencies.health)
      return reply
        .code(200)
        .send({ data: { module: "kokoro-billing", status: "ready" } });
    try {
      await dependencies.health.postgres();
    } catch {
      return reply.code(503).send({
        error: {
          code: "billing.dependencies_not_ready",
          message: "billing dependencies are not ready",
        },
      });
    }
    ...
    return reply.code(200).send({
      data: {
        ...
        status: "ready",
        dependencies: { postgres: "ok", redis },
      },
    });
  });
    ...
    try {
      ...
      if (isUnknownRecord(body.error)) {
        ...
        return JSON.stringify({
          ...
          meta: { ...meta, request_id: request.id },
        });
      }
  ...
  app.addContentTypeParser(
    ...
    (request, body, done) => {
      ...
      try {
        ...
        done(new Error("billing.invalid_json"));
      }
    },
  );
  app.addContentTypeParser(
    ...
    if (!context || !allowed.includes(context.serviceId)) {
      void reply.code(403).send({
        error: {
          ...
          message: "registered service identity required",
        },
      });
      return null;
    }
  ...
  const admissionWire = (result: BillingAdmissionResult) => ({
    ...
    mode: result.mode,
    price_policy_revision_id: result.pricePolicyRevisionId,
  ...
  app.get("/v1/commerce/catalog", async (request, reply) => {
    ...
    if (!dependencies.catalog)
      return reply.code(503).send({
        error: {
          code: "billing.catalog_not_configured",
          message: "catalog is not configured",
        },
      });
    ...
    try {
      const page = await dependencies.catalog.listSellable(
        context.tenantId,
        parsed.data.limit,
        parsed.data.cursor,
      );
  ...
  app.get("/v1/billing/me/credit-account", async (request, reply) => {
    ...
    try {
      const account = await dependencies.account.getForSubject(
        context.tenantId,
        context.subjectId,
      );
      if (!account)
      ...
      return reply
        .code(200)
        .send({ data: toSnakeCase(account), meta: { request_id: request.id } });
    } catch (error) {
      return sendError(reply, error);
    }
  });
  ...
  app.get("/v1/billing/me/credit-ledger", async (request, reply) => {
    ...
    if (!dependencies.accountRead)
      return reply.code(503).send({
        error: {
          code: "billing.account_read_not_configured",
          message: "credit ledger is not configured",
        },
      });
    const parsed = pageQuerySchema.safeParse(request.query);
    if (!parsed.success)
    ...
    try {
      const ledger = await dependencies.accountRead.ledgerForSubject(
        context.tenantId,
        context.subjectId,
        parsed.data.limit,
        parsed.data.cursor,
      ...
      return reply
        .code(200)
        .send({ data: toSnakeCase(ledger), meta: { request_id: request.id } });
    } catch (error) {
      return sendError(reply, error);
    }
  });
  ...
  app.get("/v1/billing/me/subscriptions", async (request, reply) => {
    ...
    if (!dependencies.subscriptionRead)
      return reply.code(503).send({
        error: {
          code: "billing.subscription_not_configured",
          message: "subscription read is not configured",
        },
      });
    const parsed = pageQuerySchema.safeParse(request.query);
    if (!parsed.success)
    ...
    try {
      const page = await dependencies.subscriptionRead.listForSubject(
        context.tenantId,
        context.subjectId,
        parsed.data.limit,
        parsed.data.cursor,
  ...
  app.post("/v1/billing/checkout", async (request, reply) => {
    ...
    if (subjectId === undefined) {
      return reply.code(403).send({
        error: {
          code: "billing.service_subject_required",
          message: "web BFF subject context required for checkout",
        },
      });
    }
    ...
    const parsed = z
      .object({
        offer_revision_id: z.string().min(1),
        amount_minor: positiveDecimalString,
        currency: z.string().regex(/^[A-Z]{3}$/u),
        quote_snapshot: z.record(z.string(), z.unknown()),
      })
      .strict()
    ...
    try {
      const result = await dependencies.checkout.create({
        offerRevisionId: parsed.data.offer_revision_id,
        amountMinor: safeDecimal(parsed.data.amount_minor, "amount_minor"),
        currency: parsed.data.currency,
        quoteSnapshot: toCamelCase(parsed.data.quote_snapshot),
        tenantId: context.tenantId,
        subjectId,
        idempotencyKey: key,
        expiresAt: new Date(Date.now() + 300_000),
      });
      const hosted = dependencies.checkout.createHostedSession
        ? await dependencies.checkout.createHostedSession(
            context.tenantId,
            result.checkoutId,
          )
        : result;
      return reply.code(201).send({
        data: toSnakeCase({
          ...hosted,
          amountMinor: String(hosted.amountMinor),
        }),
        meta: { request_id: request.id },
      });
    } catch (error) {
  ...
  app.post("/v1/internal/entitlement/admissions", async (request, reply) => {
    const context = await targetInternal(request, reply, [
      "agent",
      "model",
      "studio",
    ]);
    ...
    if (!dependencies.admission)
      return reply.code(503).send({
        error: {
          code: "billing.admission_not_configured",
          message: "admission is not configured",
        },
      });
    const key = requireIdempotency(request, reply);
    if (!key) return;
    ...
    await markIdempotencyKeySeen(dependencies, request, key, context.tenantId);
    try {
      const result = await dependencies.admission.create({
        tenantId: context.tenantId,
        billingSubject: parsed.data.billing_subject,
        payerRef: parsed.data.payer_ref,
        featureKey: parsed.data.feature_key,
        surface: parsed.data.surface,
        invocationId: parsed.data.invocation_id,
        executionId: parsed.data.execution_id,
        meterKind: parsed.data.meter_kind,
        ...(parsed.data.requested_model_tier === undefined
          ? {}
          : { requestedModelTier: parsed.data.requested_model_tier }),
        idempotencyKey: key,
      });
      return reply.code(201).send({
        data: admissionWire(result),
        meta: { request_id: request.id },
      });
    } catch (error) {
      return sendError(reply, error);
  ...
  app.post<{ Params: { admissionId: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        const result = await dependencies.admission.capture(
          context.tenantId,
          request.params.admissionId,
          {
            invocationId: parsed.data.invocation_id,
            executionId: parsed.data.execution_id,
            acceptedProviderRef: parsed.data.accepted_provider_ref,
            acceptedAt: new Date(parsed.data.accepted_at),
            serviceReceipt: parsed.data.service_receipt,
            receiptSchemaVersion: parsed.data.receipt_schema_version,
          },
          key,
        );
        return reply.code(200).send({
    ...
    async (request, reply) => {
      ...
      try {
        const result = await dependencies.admission.release({
          ...
          ...(body.data.service_receipt === undefined
            ? {}
            : { serviceReceipt: body.data.service_receipt }),
          idempotencyKey: key,
        });
        return reply.code(200).send({
          data: admissionWire(result),
  ...
  app.post("/v1/internal/billing/execution-events", async (request, reply) => {
    ...
    await markIdempotencyKeySeen(dependencies, request, key, context.tenantId);
    try {
      const result = await dependencies.admission.recordExecutionEvent({
        tenantId: context.tenantId,
      ...
      return reply.code(202).send({
        data: { event_id: result.eventId, status: result.status },
        meta: { request_id: request.id },
      });
    } catch (error) {
      return sendError(reply, error);
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const result = await dependencies.settlement.recordSettlement({
          settlementId: parsed.data.settlement_id,
          externalPaymentRef: parsed.data.external_payment_ref,
          amountMinor: safeDecimal(parsed.data.amount_minor, "amount_minor"),
          currency: parsed.data.currency,
          tenantId: context.tenantId,
          idempotencyKey: key,
  ...
  app.post("/v1/internal/payment/refunds/accept", async (request, reply) => {
    ...
    await markIdempotencyKeySeen(dependencies, request, key, context.tenantId);
    try {
      const reversalId = await dependencies.reversal.recordReversal({
        tenantId: context.tenantId,
        settlementId: parsed.data.settlement_id,
        externalReversalRef: parsed.data.external_ref,
        amountMinor: safeDecimal(parsed.data.amount_minor, "amount_minor"),
        reason: `${parsed.data.allocation_mode}:${parsed.data.reason}`,
        idempotencyKey: key,
      });
      return reply.code(202).send({
        data: { refund_id: reversalId, status: "accepted" },
        meta: { request_id: request.id },
      });
    } catch (error) {
      return sendError(reply, error);
  ...
  app.post("/v1/admin/billing/refunds", async (request, reply) => {
    ...
    if (!context)
      return reply.code(403).send({
        error: { code: "billing.forbidden", message: "admin role required" },
      });
    const key = requireIdempotency(request, reply);
    if (!key) return;
    const parsed = targetRefundSchema.safeParse(request.body);
    ...
    await markIdempotencyKeySeen(dependencies, request, key, context.tenantId);
    try {
      const refundId = await dependencies.reversal.recordReversal({
        tenantId: context.tenantId,
        settlementId: parsed.data.settlement_id,
        externalReversalRef: parsed.data.external_ref,
        amountMinor: safeDecimal(parsed.data.amount_minor, "amount_minor"),
      ...
      return reply.code(202).send({
        data: { refund_id: refundId, status: "accepted" },
        meta: { request_id: request.id },
      });
    } catch (error) {
      return sendError(reply, error);
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        return reply.code(202).send({
          data: toSnakeCase(
            await dependencies.usage.expireExpiredHolds({
              tenantId: context.tenantId,
  ...
  app.post<{ Params: { provider: string } }>(
    ...
    async (request, reply) => {
      const providerResult = paymentProviderSchema.safeParse(
        request.params.provider,
      );
      if (!providerResult.success)
        return reply.code(400).send({
          error: {
            code: "billing.provider_not_supported",
            message: "provider is not enabled by the production contract",
          },
        });
      const provider = providerResult.data;
      if (!(await dependencies.auth.webhook(request)))
        return reply.code(401).send({
          error: {
            code: "billing.provider_event_invalid",
            message: "provider signature required",
          },
        });
      ...
      try {
        ...
        const providerAccountRef =
          parsed?.providerAccountRef ??
          dependencies.resolveWebhookAccountRef?.(provider) ??
          null;
        const tenantId = dependencies.resolveWebhookTenant
          ? providerAccountRef === null
            ? null
            : await dependencies.resolveWebhookTenant(
                provider,
                providerAccountRef,
              )
        ...
        if (parsed?.payloadTenantId && parsed.payloadTenantId !== tenantId)
          throw new Error("billing.tenant_mismatch");
        ...
        if (
          ...
          throw new Error("billing.provider_payload_invalid");
        const result = await dependencies.webhook.accept({
          tenantId: tenantId,
          provider,
          providerAccountRef,
          externalEventId,
