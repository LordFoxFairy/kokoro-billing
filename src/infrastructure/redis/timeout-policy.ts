  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_resolve, reject) => {
        timer = setTimeout(() => reject(new RedisOperationTimeoutError(name, timeoutMs)), timeoutMs);
      }),
    ]);
  } finally {
...
const isRetryableRedisError = (error: unknown): boolean => {
  ...
  if (error instanceof Error && ['ConnectionTimeoutError', 'SocketClosedUnexpectedlyError', 'TimeoutError'].includes(error.name)) return true;
  const code = redisErrorCode(error);
  return code !== undefined && ['ECONNRESET', 'ECONNREFUSED', 'EPIPE', 'ETIMEDOUT', 'ENETUNREACH', 'EHOSTUNREACH'].includes(code);
};
...
export const runIdempotentRedisOperation = async <T>(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (remainingMs <= 0) throw new RedisOperationTimeoutError(name, policy.overallTimeoutMs);
    try {
      return await withTimeout(operation(), Math.min(policy.readTimeoutMs, remainingMs), name);
    } catch (error) {
      lastError = error;
...
export const connectRedisWithDeadline = async (
  ...
  await withTimeout(operation, Math.min(policy.connectTimeoutMs, policy.overallTimeoutMs), 'connect');
};
