...
import type { BillingAuth, BillingAdminContext, BillingInternalContext, BillingUserContext } from '../../interfaces/http/server.js';
...
const fixtureUser = (request: FastifyRequest): BillingUserContext | null => {
  ...
  const subjectId = headerValue(request, 'x-kokoro-subject');
...
const trustedInternal = (request: FastifyRequest, secret: string): BillingInternalContext | null => {
  ...
  return headerValue(request, 'x-kokoro-internal-secret') === secret && tenantId !== undefined && serviceId !== undefined && registeredInternalServices.has(serviceId)
    ? { tenantId, serviceId }
    : null;
};
...
const trustedAdmin = (request: FastifyRequest, secret: string): BillingAdminContext | null => {
  ...
  return service === 'admin' && headerValue(request, 'x-kokoro-proxy-secret') === secret
