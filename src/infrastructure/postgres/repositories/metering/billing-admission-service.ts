...
} from "../../../../domain/payment/services/billing-state-machine.js";
...
type AdmissionRow = RowDataPacket & {
  ...
  execution_id: string;
  feature_key: string;
  billing_subject_kind: string;
  billing_subject_ref: string;
  payer_ref: string;
  surface: string;
  meter_kind: string;
  requested_model_tier: string | null;
  price_policy_revision_id: string | null;
...
const admissionResultSchema = z
  ...
  .strict();
const executionEventResultSchema = z
  .object({
...
export class BillingAdmissionService {
  ...
  public async create(
    input: CreateBillingAdmissionInput,
    ...
    if (existing) {
      if (
        existing.billing_subject_kind !== input.billingSubject.kind ||
    ...
    const [rates] = await this.connection.execute<
      (RowDataPacket & {
    ...
    if (amountMicros === 0) {
      await this.connection.execute(
        ...
        [
          admissionId,
          input.tenantId,
          input.billingSubject.kind,
          input.billingSubject.ref,
          input.payerRef,
          input.featureKey,
          input.surface,
          input.invocationId,
          input.executionId,
          input.meterKind,
          input.requestedModelTier ?? null,
          rate.usage_price_revision_id,
          input.idempotencyKey,
        ],
      );
      const result: BillingAdmissionResult = {
        admissionId,
        holdId: null,
        mode: "included",
        pricePolicyRevisionId: rate.usage_price_revision_id,
        amountMicros: "0",
        currency: "CRD",
        status: "accepted_without_charge",
      };
      const accepted = {
        ...result,
    ...
    const accountId = await this.ensureCreditAccount(
      input.tenantId,
      input.billingSubject.ref,
    );
  ...
  public async capture(
    ...
    if (Number.isNaN(receipt.acceptedAt.getTime()))
      throw new Error("billing.invalid_request");
    const claim = await this.claimReceipt(
      tenantId,
      "internal",
      CAPTURE_COMMAND,
    ...
    if (
      admission.invocation_id !== receipt.invocationId ||
      admission.execution_id !== receipt.executionId
    )
      throw new Error("billing.tenant_mismatch");
    if (admission.status === "captured") {
      const result = {
        ...this.toResult(admission),
        status: "accepted_without_charge" as const,
      };
      await this.finishReceipt(
        claim.receiptId,
    ...
    if (admission.status === "released" || admission.status === "rejected")
      throw new Error("billing.admission_not_active");
  ...
  public async release(
    ...
    if (admission.invocation_id !== input.invocationId)
      throw new Error("billing.tenant_mismatch");
  ...
  public async recordExecutionEvent(input: ExecutionEventInput): Promise<{
    readonly eventId: string;
    readonly status: "received" | "processed";
  }> {
    ...
    await this.connection.execute(
      ...
      [
        ...
        input.receipt === undefined ? null : canonicalJson(input.receipt),
        payloadHash,
      ],
    );
    ...
    if (!row)
      throw new PersistedDataInvariantError(
        "billing.execution_event_not_found_after_insert",
      );
  ...
  public async processExecutionEvent(
    tenantId: string,
    eventId: string,
  ): Promise<void> {
    const [events] = await this.connection.execute<RowDataPacket[]>(
      `SELECT event_id, tenant_id, execution_id, invocation_id, event_type,
    ...
    const event = events[0] as
      | (RowDataPacket & {
          event_type: ExecutionEventInput["eventType"];
          invocation_id: string;
          execution_id: string;
    ...
    const [admissions] = await this.connection.execute<AdmissionRow[]>(
      `SELECT admission_id, tenant_id, invocation_id, execution_id, feature_key,
                billing_subject_kind, billing_subject_ref, payer_ref, surface, meter_kind,
                requested_model_tier, price_policy_revision_id, amount_micros, mode, status,
                hold_id, accepted_provider_ref, accepted_at
           FROM entitlement_billing_admission
          WHERE tenant_id = $1 AND invocation_id = $2`,
      [tenantId, event.invocation_id],
    );
    const admission = admissions[0];
    if (!admission) throw new Error("billing.admission_not_found");
    if (event.event_type === "execution.accepted") {
      const parsedReceipt =
        event.receipt_json === null
          ? {}
          : parsePersistedJson(
              event.receipt_json,
              jsonRecordSchema,
              "billing.execution_receipt_invalid",
            );
      const acceptedProviderRef =
        parsedReceipt.provider_operation_ref ?? "event";
      if (typeof acceptedProviderRef !== "string")
        throw new Error("billing.execution_receipt_invalid");
      const acceptedAt =
        typeof parsedReceipt.accepted_at === "string"
          ? new Date(parsedReceipt.accepted_at)
      ...
      await this.capture(
        ...
        admission.admission_id,
        {
          ...
          acceptedProviderRef,
          acceptedAt,
          serviceReceipt: parsedReceipt,
          receiptSchemaVersion: event.receipt_schema_version,
        },
        `execution-event:${eventId}`,
      );
    } else if (
      event.event_type === "execution.rejected" ||
      ...
      const lockedAdmission = await this.getAdmission(
        tenantId,
        admission.admission_id,
      );
      if (lockedAdmission.status === "held")
        await this.markAdmission(tenantId, admission.admission_id, "unknown");
    }
    await this.connection.execute(
      "UPDATE entitlement_execution_event SET status = 'processed', processed_at = CURRENT_TIMESTAMP(3) WHERE tenant_id = $1 AND event_id = $2 AND status = 'received'",
      [tenantId, eventId],
    );
  }
  ...
  private async getAdmission(
    tenantId: string,
    admissionId: string,
  ): Promise<AdmissionRow> {
    const [rows] = await this.connection.execute<AdmissionRow[]>(
      `SELECT admission_id, tenant_id, invocation_id, execution_id, feature_key,
                billing_subject_kind, billing_subject_ref, payer_ref, surface, meter_kind,
                requested_model_tier, price_policy_revision_id, amount_micros, mode, status,
                hold_id, accepted_provider_ref, accepted_at
  ...
  private async markAdmission(
    ...
    const [result] = await this.connection.execute<ResultSetHeader>(
      ...
      [
        status,
        receipt?.acceptedProviderRef ?? null,
        receipt?.acceptedAt ?? null,
        receipt ? canonicalJson(receipt.serviceReceipt) : null,
        tenantId,
        admissionId,
        current.status,
      ],
    ...
    if (
      ...
      (await this.getAdmission(tenantId, admissionId)).status !== status
    )
      throw new Error("billing.admission_transition_conflict");
  }
  ...
  private toResult(
    ...
    return {
      ...
      holdId: row.hold_id,
      mode: row.mode,
      pricePolicyRevisionId: row.price_policy_revision_id,
      amountMicros: String(row.amount_micros),
      currency: "CRD",
      status:
        row.status === "held" || row.status === "unknown"
          ? "held"
          : row.status === "captured"
            ? "accepted_without_charge"
            : "rejected",
    };
  }
  ...
  private async claimReceipt<T>(
    ...
    if (row.payload_hash !== hash || row.command_identity !== identity)
      throw new Error("billing.idempotency_conflict");
    if (row.status === "succeeded") {
      return {
        kind: "replay",
        result: parsePersistedJson(
          row.result_json,
          resultSchema,
    ...
    if (row.status === "failed") throw new Error("billing.command_failed");
    if (row.status === "unknown" || inserted.affectedRows !== 1)
