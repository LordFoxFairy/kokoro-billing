...
const usagePriceRateSchema = z.object({
  featureKey: z.string().min(1), labelKey: z.string().nullable().optional(), modelBindingId: z.string().nullable().optional(),
  inputMicrosPerMillion: z.number().int().nonnegative(), outputMicrosPerMillion: z.number().int().nonnegative(),
  cachedMicrosPerMillion: z.number().int().nonnegative().optional(), reservationMicros: z.number().int().positive(),
}).strict();
const publishedUsagePricingSchema = z.object({
  pricingRevisionId: z.string().min(1), revision: z.number().int().positive(), effectiveFrom: z.string().datetime({ offset: true }),
  rates: z.array(usagePriceRateSchema),
}).strict();

const parsePublishedUsagePricing = (value: unknown): PublishedUsagePricing => {
  const parsed = parsePersistedJson(value, publishedUsagePricingSchema, 'billing.command_result_invalid');
  return {
    ...
    rates: parsed.rates.map((rate) => ({
      ...
      ...(rate.cachedMicrosPerMillion === undefined ? {} : { cachedMicrosPerMillion: rate.cachedMicrosPerMillion }),
      reservationMicros: rate.reservationMicros,
    })),
  };
};

const identity = (rate: UsagePriceRateInput): string => `${rate.featureKey}\u0000${rate.labelKey ?? ''}`;
...
export class UsagePricingAdminService {
  ...
  public async publish(input: PublishUsagePricingInput): Promise<PublishedUsagePricing> {
    if (input.rates.length === 0) throw new Error('billing.usage_price_rates_required');
    if (input.rates.some((rate) => [rate.inputMicrosPerMillion, rate.outputMicrosPerMillion, rate.cachedMicrosPerMillion ?? 0, rate.reservationMicros].some((value) => !Number.isSafeInteger(value) || value < 0))) throw new Error('billing.usage_price_invalid');
    if (input.rates.some((rate) => rate.reservationMicros <= 0 || rate.featureKey.trim() === '')) throw new Error('billing.usage_price_invalid');
    const identities = new Set(input.rates.map(identity));
    if (identities.size !== input.rates.length) throw new Error('billing.usage_price_duplicate_rate');
    const payloadHash = createHash('sha256').update(JSON.stringify({ ...input, effectiveFrom: input.effectiveFrom.toISOString(), idempotencyKey: undefined })).digest('hex');
    ...
    try {
      ...
      const result: PublishedUsagePricing = { pricingRevisionId, revision, effectiveFrom: input.effectiveFrom.toISOString(), rates: input.rates };
