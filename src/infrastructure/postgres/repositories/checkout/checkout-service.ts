...
import type { Checkout, CreateCheckoutInput } from '../../../../application/checkout/commands/checkout-service.js';
...
const hashQuote = (input: CreateCheckoutInput): string => canonicalJsonDigest({
...
export class CheckoutService {
  public constructor(private readonly connection: SqlConnection, private readonly options: { readonly hostedProvider?: HostedCheckoutProvider; readonly publicBaseUrl?: string } = {}) {}
  ...
  public async create(input: CreateCheckoutInput): Promise<Checkout> {
    ...
    if (existing) {
      if (existing.quote_hash !== quoteHash) throw new Error('billing.idempotency_conflict');
      return toCheckout(existing);
    }

    if (Number.isNaN(input.expiresAt.getTime())) throw new Error('billing.checkout_quote_invalid');
    const [clockRows] = await this.connection.execute<(RowDataPacket & { database_now: Date | string })[]>(
      'SELECT CURRENT_TIMESTAMP(3) AS database_now',
    );
    const databaseNow = clockRows[0]?.database_now;
    if (databaseNow === undefined) throw new PersistedDataInvariantError('billing.database_clock_unavailable');
    if (input.expiresAt.getTime() <= new Date(databaseNow).getTime()) throw new Error('billing.quote_expired');
    ...
    await this.connection.execute(
      ...
      [checkoutId, input.tenantId, input.subjectId, input.idempotencyKey, input.offerRevisionId, quoteHash, canonicalJson(input.quoteSnapshot), input.amountMinor, input.currency, input.expiresAt],
    );
  ...
  public async createHostedSession(tenantId: string, checkoutId: string): Promise<Checkout> {
    ...
    const quote = parsePersistedJson(row.quote_snapshot_json, quoteSnapshotSchema, 'billing.checkout_quote_persisted_invalid');
