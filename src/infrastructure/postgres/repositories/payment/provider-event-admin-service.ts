...
type ReceiptRow = RowDataPacket & {
  payload_hash: string;
  status: string;
  result_json: string | ProviderEventRetryResult | null;
};
type EventRow = RowDataPacket & {
  provider_event_id: string;
  processing_status: ProviderEventRetryResult["processingStatus"];
};
...
type ProviderEventCursor = {
  readonly version: 1;
  readonly scope: "payment.provider-event.admin";
  readonly tenantId: string;
  readonly status: ProviderEventRetryResult["processingStatus"] | null;
  readonly receivedAt: string;
  readonly providerEventId: string;
};
const providerEventRetryResultSchema = z
  .object({
    providerEventId: z.string().min(1),
    processingStatus: z.enum(["received", "processed", "ignored", "failed"]),
  })
  .strict();
const providerEventCursorSchema = z
  .object({
    version: z.literal(1),
    scope: z.literal("payment.provider-event.admin"),
...
export class ProviderEventAdminService {
  ...
  public async list(input: {
    ...
    const cursor =
      input.cursor === undefined
        ? undefined
        : decodeProviderEventCursor(
            input.cursor,
            input.tenantId,
            input.status ?? null,
          );
    const statusPredicate =
      input.status === undefined ? "" : "AND processing_status = $2";
    const cursorOffset = input.status === undefined ? 2 : 3;
    const cursorPredicate =
      cursor === undefined
        ? ""
        : `AND (received_at < $${cursorOffset} OR (received_at = $${cursorOffset + 1} AND provider_event_id < $${cursorOffset + 2}))`;
    ...
    return {
      items,
      nextCursor: encodeProviderEventCursor({
        version: 1,
        scope: "payment.provider-event.admin",
  ...
  public async retry(
    input: ProviderEventRetryInput,
  ): Promise<ProviderEventRetryResult> {
    if (input.reason.trim() === "")
      throw new Error("billing.admin_reason_required");
    ...
    try {
      ...
      if (prior[0]) {
        ...
        if (prior[0].status === "processing" || prior[0].status === "unknown")
        ...
        const result = parsePersistedJson(
          prior[0].result_json,
          providerEventRetryResultSchema,
      ...
      let result: ProviderEventRetryResult = {
      ...
      if (
        ...
        await this.connection.execute(
          ...
          [
            randomUUID(),
            JSON.stringify({ providerEventId: event.provider_event_id }),
            input.tenantId,
...
function decodeProviderEventCursor(
  value: string,
  tenantId: string,
  status: ProviderEventRetryResult["processingStatus"] | null,
): ProviderEventCursor {
  try {
