...
import { parseProviderWebhook } from "../../../../application/payment/ports/provider-registry.js";
import type { ProviderRegistry } from "../../../../application/payment/ports/provider-registry.js";
import { PAYMENT_WEBHOOK_EVENT } from "../../../../application/payment/ports/provider-types.js";
import type { BillingSettlementService } from "./billing-settlement-service.js";
import type { BillingReversalService } from "../refund/billing-reversal-service.js";
import type { SubscriptionGrantService } from "../credit/subscription-grant-service.js";
import type { ParsedSubscriptionEvent } from "../../../../application/payment/ports/provider-types.js";
...
const object = (
  ...
  parsePersistedJson(
    value,
    jsonRecordSchema,
    "billing.provider_payload_invalid",
  );

const snapshot = (
  value: string | Record<string, unknown>,
): { programKey: string; creditMicros: number } => {
  const data = object(value);
  const creditMicros = data.creditMicros;
  if (
    typeof data.key !== "string" ||
    (typeof creditMicros !== "string" && typeof creditMicros !== "number") ||
    !/^\d+$/u.test(String(creditMicros))
  )
    throw new Error("billing.checkout_quote_invalid");
  return {
    programKey: data.key,
    creditMicros: readSafeInteger(String(creditMicros), "credit_micros"),
  };
};
...
export class ProviderEventProcessor {
  public constructor(
    ...
    private readonly settlement: Pick<
      BillingSettlementService,
      "recordSettlement" | "fulfillSettlement"
    >,
    private readonly reversal: Pick<
      BillingReversalService,
      "recordReversal" | "reverseCredits"
    >,
  ...
  public async process(providerEventId: string): Promise<void> {
    ...
    await this.connection.execute(
      `UPDATE payment_provider_event SET processing_attempts = processing_attempts + 1 WHERE tenant_id = $1 AND provider_event_id = $2`,
      [event.tenant_id, providerEventId],
    );
    try {
      const parsed = parseProviderWebhook(
        this.providers,
        event.provider,
        object(event.payload_json),
      );
      if (parsed.eventType === PAYMENT_WEBHOOK_EVENT.paymentSucceeded) {
        ...
        await this.processRefund(
          ...
          parsed.orderId,
          parsed.eventId,
          parsed.externalReversalRef,
          parsed.refundAmountMinor,
        );
        await this.markProcessed(event.tenant_id, providerEventId);
      } else if (
        parsed.eventType === PAYMENT_WEBHOOK_EVENT.subscriptionUpdated
      ) {
        if (!parsed.subscription)
          throw new Error("billing.subscription_event_invalid");
  ...
  private async processPayment(
    ...
    this.assertProviderAccount(
      checkout.provider_account_ref,
      event.provider_account_ref,
    );
    const plan = snapshot(checkout.quote_snapshot_json);
    const settlementId = uuidFromKey(
      `payment-settlement:${event.tenant_id}:${event.provider}:${externalEventId}`,
    );
    await this.settlement.recordSettlement({
      ...
      externalPaymentRef:
        externalPaymentRef ?? `${event.provider}:${externalEventId}`,
      amountMinor: readSafeInteger(
    ...
    if (plan.creditMicros > 0)
      await this.settlement.fulfillSettlement({
        settlementId,
        tenantId: event.tenant_id,
        accountId,
        subjectId: checkout.subject_id,
        programKey: plan.programKey,
        grantMicros: plan.creditMicros,
      });
  }
  ...
  private async processRefund(
    ...
    >(
      `SELECT settlement_id, amount_minor FROM payment_settlement WHERE tenant_id = $1 AND provider = $2 AND checkout_id = $3 AND status = 'succeeded' ORDER BY created_at DESC, settlement_id DESC LIMIT 1 FOR UPDATE`,
      [event.tenant_id, event.provider, checkout.checkout_id],
    );
    const settlement = settlements[0];
    if (!settlement) throw new Error("billing.settlement_not_found");
    const [grants] = await this.connection.execute<GrantRow[]>(
      `SELECT g.credit_account_id, g.original_micros FROM entitlement_credit_grant g JOIN entitlement_acquisition a ON a.tenant_id = g.tenant_id AND a.source_kind = 'payment_settlement' AND a.source_ref = g.source_ref WHERE g.tenant_id = $1 AND g.source_ref = $2 LIMIT 1`,
      [event.tenant_id, settlement.settlement_id],
    );
    const grant = grants[0];
    if (!grant && plan.creditMicros > 0)
      throw new Error("billing.refund_grant_not_found");
    if (!grant) return;
    const amountMinor =
      refundAmountMinor ??
      readSafeInteger(settlement.amount_minor, "settlement_amount_minor");
    const reversalId = await this.reversal.recordReversal({
      provider: event.provider,
      tenantId: event.tenant_id,
      settlementId: settlement.settlement_id,
      externalReversalRef:
        externalReversalRef ?? `${event.provider}:${externalEventId}`,
      amountMinor,
      reason: "provider_refund",
      idempotencyKey: `provider-refund:${event.provider}:${externalEventId}`,
    });
    // Leave the credit amount allocation to the locked reversal transaction so concurrent partial refunds cannot over-reverse.
    await this.reversal.reverseCredits({
      tenantId: event.tenant_id,
      reversalId,
      settlementId: settlement.settlement_id,
      accountId: grant.credit_account_id,
    });
  }

  private async processSubscription(
    event: EventRow,
    subscription: ParsedSubscriptionEvent,
  ): Promise<void> {
    if (
    ...
    if (!subscription.currentPeriodStart || !subscription.currentPeriodEnd) {
      if (subscription.status === "active" && subscription.grantCredits)
        throw new Error("billing.subscription_period_missing");
    }
    const [revisions] = await this.connection.execute<
      (RowDataPacket & { offer_key: string; credit_micros: number | string })[]
    >(
      `SELECT o.offer_key, r.credit_micros
         FROM entitlement_offer_revision r JOIN entitlement_offer o ON o.offer_id = r.offer_id AND o.tenant_id = r.tenant_id
    ...
    const providerSubscriptionId = uuidFromKey(
      `provider-subscription:${event.tenant_id}:${event.provider}:${subscription.providerSubscriptionId}`,
    );
    const [existingSubscriptions] = await this.connection.execute<
      (RowDataPacket & { provider_account_id: string | null })[]
    >(
    ...
    const existingProviderAccountId =
      existingSubscriptions[0]?.provider_account_id ?? null;
    if (
      existingProviderAccountId !== null &&
      existingProviderAccountId !== providerAccountId
    )
      throw new Error("billing.provider_account_mismatch");
    await this.connection.execute(
    ...
    const periodId = uuidFromKey(
      `subscription-period:${event.tenant_id}:${event.provider}:${periodKey}`,
    );
    await this.connection.execute(
      ...
      [
        ...
        subscription.status === "active" ? "open" : "unknown",
      ],
    );
    const amountMicros = readSafeInteger(
      revision.credit_micros,
      "subscription_credit_micros",
    );
    await this.connection.execute(
    ...
    if (subscription.grantCredits) {
      if (amountMicros > 0) {
        ...
        await this.subscriptionGrant.grant({
          tenantId: event.tenant_id,
          subjectId: subscription.teamId,
          accountId,
          periodId,
          programKey: revision.offer_key,
          amountMicros,
  ...
  private assertProviderAccount(
    checkoutAccountRef: string | null,
    eventAccountRef: string | null,
  ): void {
    if (
      (checkoutAccountRef !== null || eventAccountRef !== null) &&
      checkoutAccountRef !== eventAccountRef
    )
      throw new Error("billing.provider_account_mismatch");
  }
  private async resolveProviderAccountId(
    event: EventRow,
  ): Promise<string | null> {
  ...
  private async getEvent(id: string): Promise<EventRow | undefined> {
    const [rows] = await this.connection.execute<EventRow[]>(
      `SELECT provider_event_id, tenant_id, provider, provider_account_ref, processing_status, payload_json FROM payment_provider_event WHERE provider_event_id = $1`,
      [id],
    );
    return rows[0];
  }
  private async getCheckout(
    ...
    const [rows] = await this.connection.execute<CheckoutRow[]>(
      `SELECT checkout_id, subject_id, amount_minor, currency, provider_account_ref, quote_snapshot_json FROM payment_checkout WHERE tenant_id = $1 AND checkout_id = $2`,
      [tenantId, checkoutId],
    );
    if (!rows[0]) throw new Error("billing.checkout_not_found");
    return rows[0];
  }
  private async accountId(
    ...
    await this.connection.execute(
      `INSERT INTO entitlement_credit_account (credit_account_id, tenant_id, subject_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
  ...
  private async markProcessed(tenantId: string, id: string): Promise<void> {
    await this.connection.execute(
      `UPDATE payment_provider_event SET processing_status = 'processed', processed_at = CURRENT_TIMESTAMP(3), last_error = NULL WHERE tenant_id = $1 AND provider_event_id = $2`,
      [tenantId, id],
    );
  }
  private async markIgnored(tenantId: string, id: string): Promise<void> {
    await this.connection.execute(
      `UPDATE payment_provider_event SET processing_status = 'ignored', processed_at = CURRENT_TIMESTAMP(3), last_error = NULL WHERE tenant_id = $1 AND provider_event_id = $2`,
      [tenantId, id],
    );
  }
  private async markFailed(
    ...
    await this.connection.execute(
      `UPDATE payment_provider_event SET processing_status = 'failed', last_error = $1 WHERE tenant_id = $2 AND provider_event_id = $3`,
      [error.slice(0, 1024), tenantId, id],
    );
  }
}
