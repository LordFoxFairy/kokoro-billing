...
const object = (value: string | Record<string, unknown>): Record<string, unknown> => parsePersistedJson(value, jsonRecordSchema, 'billing.provider_payload_invalid');
...
export class ProviderEventProcessor {
  ...
  private async processRefund(event: EventRow, checkoutId: string | null, externalEventId: string, externalReversalRef: string | null, refundAmountMinor: number | null): Promise<void> {
    ...
    this.assertProviderAccount(checkout.provider_account_ref, event.provider_account_ref);
    const plan = snapshot(checkout.quote_snapshot_json);
    const [settlements] = await this.connection.execute<(RowDataPacket & { settlement_id: string; amount_minor: number | string })[]>(`SELECT settlement_id, amount_minor FROM payment_settlement WHERE tenant_id = $1 AND provider = $2 AND checkout_id = $3 AND status = 'succeeded' ORDER BY created_at DESC, settlement_id DESC LIMIT 1 FOR UPDATE`, [event.tenant_id, event.provider, checkout.checkout_id]);
  ...
  private async markProcessed(tenantId: string, id: string): Promise<void> { await this.connection.execute(`UPDATE payment_provider_event SET processing_status = 'processed', processed_at = CURRENT_TIMESTAMP(3), last_error = NULL WHERE tenant_id = $1 AND provider_event_id = $2`, [tenantId, id]); }
  private async markIgnored(tenantId: string, id: string): Promise<void> { await this.connection.execute(`UPDATE payment_provider_event SET processing_status = 'ignored', processed_at = CURRENT_TIMESTAMP(3), last_error = NULL WHERE tenant_id = $1 AND provider_event_id = $2`, [tenantId, id]); }
