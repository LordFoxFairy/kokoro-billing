  ...
  public async grant(input: AdminGrantInput): Promise<AdminGrantResult> {
    ...
    try {
      ...
      if (prior[0]) {
        ...
        if (existing.status === 'processing' || existing.status === 'unknown') throw new Error('billing.command_unknown');
        if (existing.status === 'failed') throw new Error('billing.command_failed');
        if (existing.status !== 'succeeded') throw new Error('billing.command_unknown');
