...
const adminGrantResultSchema = z
  .object({ grantId: z.string().min(1), journalId: z.string().min(1) })
  .strict();
...
export class AdminGrantService {
  ...
  public async grant(input: AdminGrantInput): Promise<AdminGrantResult> {
    ...
    const payloadHash = createHash("sha256")
      .update(JSON.stringify({ ...input, operatorId: undefined }))
      .digest("hex");
    ...
    try {
      ...
      if (prior[0]) {
        const existing = prior[0] as {
          payload_hash: string;
          result_json: string | AdminGrantResult | null;
          status: string;
        };
      ...
      const account = accounts[0] as
        { credit_account_id: string; subject_id: string } | undefined;
      if (!account) throw new Error("billing.credit_account_not_found");
      if (account.subject_id !== input.subjectId)
