...
export type { QueryResult, ResultSetHeader, RowDataPacket, SqlConnection } from './database.js';

type TransactionState = { readonly client: PoolClient; depth: number };
type BillingContext = { transactions: Map<PostgresConnection, TransactionState> };
...
class PostgresConnection implements SqlConnection {
  ...
  private async runQuery<T extends QueryResult>(sql: string, values: unknown[] | undefined): Promise<[T, unknown[]]> {
    const result = await (await this.client()).query<QueryResultRow>(sql, values);
  ...
  public query<T extends QueryResult = RowDataPacket[]>(sql: string, values?: unknown[]): Promise<[T, unknown[]]> { return this.runQuery<T>(sql, values); }
  public execute<T extends QueryResult = RowDataPacket[]>(sql: string, values?: unknown[]): Promise<[T, unknown[]]> { return this.runQuery<T>(sql, values); }
  ...
  public async beginTransaction(): Promise<void> {
    ...
    const context = billingContext.getStore() ?? { transactions: new Map<PostgresConnection, TransactionState>() };
    ...
    try { await client.query('BEGIN'); context.transactions.set(this, { client, depth: 1 }); }
  ...
  public async commit(): Promise<void> {
    ...
    if (transaction.depth > 1) {
      await transaction.client.query(`RELEASE SAVEPOINT billing_sp_${transaction.depth}`);
      transaction.depth -= 1;
      return;
    }
    try { await transaction.client.query('COMMIT'); }
    finally { context.transactions.delete(this); transaction.client.release(); }
  }
...
export function createBillingConnection(uri: string): Promise<SqlConnection> { return Promise.resolve(new PostgresConnection(uri)); }
export const createDedicatedBillingConnection = (uri: string): Promise<SqlConnection> => Promise.resolve(new PostgresConnection(uri));
