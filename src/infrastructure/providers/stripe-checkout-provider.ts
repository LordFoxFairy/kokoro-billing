...
import type { HostedCheckoutInput, HostedCheckoutProvider, HostedCheckoutSession } from '../../application/checkout/ports/hosted-checkout-provider.js';
...
class StripeConnectTimeoutAgent extends HttpsAgent {
  ...
  public override createConnection(
    ...
    const timer = setTimeout(() => stream.destroy(new Error('billing.provider_connect_timeout')), this.connectTimeoutMs);
...
export class StripeCheckoutProvider implements HostedCheckoutProvider {
  ...
  public constructor(secretKey: string, accountRef?: string, timeouts: StripeTimeoutPolicy = DEFAULT_STRIPE_TIMEOUT_POLICY) {
  ...
  public async createSession(input: HostedCheckoutInput): Promise<HostedCheckoutSession> {
    ...
    const deadline = new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => reject(new Error('billing.provider_timeout')), this.overallTimeoutMs);
    });
