...
function stripeRefundFacts(value: Record<string, unknown> | undefined): {
  externalReversalRef: string;
  refundAmountMinor: number | null;
} {
  const refunds = stripeRefundCollectionSchema.safeParse(value?.refunds);
  const latest = refunds.success ? refunds.data.data[0] : undefined;
  return {
    externalReversalRef:
    ...
    refundAmountMinor:
      minorAmount(latest?.amount) ??
      minorAmount(value?.amount_refunded ?? value?.amount),
  };
}
...
export class StripeWebhookProvider implements PaymentWebhookProvider {
  ...
  constructor(options: StripeProviderOptions = {}) {
    ...
    // The SDK requires a constructor value even though webhook verification uses
    // the per-account secret passed to verifySignature and makes no API calls.
    this.webhooks = new Stripe("unused-webhook-client-key").webhooks;
  }
  ...
  parseEvent(payload: unknown): ParsedWebhookEvent {
    ...
    if (
      ...
      return {
        ...
        externalReversalRef: null,
        refundAmountMinor: null,
        orderId: metadata.orderId ?? metadata.checkoutId ?? null,
        subscription: null,
      };
    }
    ...
    if (type === "charge.refunded") {
      ...
      return {
        ...
        providerAccountRef,
        externalPaymentRef: null,
        externalReversalRef: refund.externalReversalRef,
    ...
    if (type.startsWith("customer.subscription.")) {
      ...
      return {
        eventId: id,
        eventType: PAYMENT_WEBHOOK_EVENT.subscriptionUpdated,
        payloadTenantId: providerPayloadTenantId(metadata),
        providerAccountRef,
        externalPaymentRef: null,
        externalReversalRef: null,
