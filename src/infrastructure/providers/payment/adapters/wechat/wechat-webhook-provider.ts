...
import { z } from "zod";
import type { ParsedWebhookEvent, PaymentWebhookProvider } from "../../../../../application/payment/ports/provider-types.js";
import { PAYMENT_WEBHOOK_EVENT, WebhookError } from "../../../../../application/payment/ports/provider-types.js";
...
const wechatResourceSchema = z.object({
  out_trade_no: z.string().optional(), metadata: webhookMetadataSchema.optional(), mchid: z.string().optional(),
  transaction_id: z.string().optional(), refund_id: z.string().optional(),
  amount: z.object({ refund: z.unknown().optional() }).passthrough().optional(),
  ciphertext: z.string().optional(), nonce: z.string().optional(), associated_data: z.string().optional(),
}).passthrough();
...
export class WechatWebhookProvider implements PaymentWebhookProvider {
  ...
  parseEvent(payload: unknown): ParsedWebhookEvent {
    ...
    if (event_type === "REFUND.SUCCESS") {
      return { eventId: id, eventType: PAYMENT_WEBHOOK_EVENT.refundSucceeded, payloadTenantId: providerPayloadTenantId(metadata), providerAccountRef: resource?.mchid ?? null, externalPaymentRef: null, externalReversalRef: resource?.refund_id ?? id, refundAmountMinor: minorAmount(resource?.amount?.refund), orderId, subscription: null };
    }
...
function decryptResource(resource: z.infer<typeof wechatEventSchema>["resource"], apiV3Key: Buffer | undefined): z.infer<typeof wechatResourceSchema> | undefined {
