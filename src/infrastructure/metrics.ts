...
export function recordHttpRequest(input: { readonly method: string; readonly route: string; readonly statusCode: number; readonly durationSeconds: number }): void {
  try {
    const labels = { method: input.method, route: input.route, status_code: String(input.statusCode) };
    httpRequestsTotal.inc(labels);
    httpRequestDurationSeconds.observe({ method: input.method, route: input.route }, input.durationSeconds);
  } catch {
    // Metrics are fail-open and must never alter request semantics.
  }
}

export function registerMetricsRoute(app: FastifyInstance, moduleName = 'billing'): void {
  if (!defaultsStarted) {
  ...
  app.get('/metrics', async (_request, reply) => {
    try {
      return reply.code(200).header('content-type', register.contentType).send(await register.metrics());
    } catch {
